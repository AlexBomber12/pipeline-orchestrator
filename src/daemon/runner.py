...
class PipelineRunner(
    ...
    async def _hydrate_status_write_failed_task_pr_ids(self) -> None:
        ...
        decoded_ok, decoded = await self._read_status_write_failed_task_ids(
            status_write_failed_tasks(self.name)
        )
        legacy_ok, legacy_decoded = await self._read_status_write_failed_task_ids(
            legacy_recovered_tasks(self.name)
        )
        if decoded_ok or legacy_ok:
    ...
    async def _read_status_write_failed_task_ids(self, key: str) -> tuple[bool, set[str]]:
