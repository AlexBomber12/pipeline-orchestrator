...
- `tasks/PR-*.md` files for what work to do (one file per PR, with structured
  headers). `tasks/QUEUE.md` is a derived artifact auto-generated by the daemon
  on every IDLE cycle for human readability, and it remains a daemon recovery
  input on startup today.
