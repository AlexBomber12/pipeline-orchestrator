...
def test_reject_text_present() -> None:
    assert "reject is final" in _render_guardrail_card().lower()
...
def test_reset_endpoint_forces_idle(
    ...
    recovery_keys = (
        "status_write_failed_tasks:example__alpha",
        "recovered_tasks:example__alpha",
    )
    for key in recovery_keys:
        redis_client.store[key] = json.dumps(["PR-384", "PR-999"])
    ...
    for key in recovery_keys:
        assert json.loads(redis_client.store[key]) == ["PR-999"]
...
def test_guardrail_reject_unknown_repo_cannot_acknowledge_success() -> None:
    with TestClient(app) as client:
