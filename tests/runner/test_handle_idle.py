...
def test_select_next_task_keeps_status_write_failed_task_in_error_without_flag(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.undo()
...
def test_select_next_task_keeps_status_write_failed_marker_for_doing_task(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.undo()
    ...
    monkeypatch.setattr(
        idle_module,
        "derive_task_status",
        lambda header, merged_pr_ids, open_prs, merged_prs, **kwargs: TaskStatus.DOING,
    )
