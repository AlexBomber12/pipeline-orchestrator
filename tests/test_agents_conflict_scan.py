...
def test_scan_flags_open_as_draft_before_continuation():
    body = "Open as draft before requesting review."
    violations = scan_for_conflicts(body)
    types = {v.violation_type for v in violations}
    assert "draft_pr_open_as" in types


def test_scan_flags_open_as_draft_pr_with_to_continuation():
    body = "Open as draft pull request to collect feedback."
    violations = scan_for_conflicts(body)
    types = {v.violation_type for v in violations}
    assert "draft_pr_open_as" in types
...
def test_scan_wrapped_force_merge_commit_guidance_not_flagged():
    body = "Use --no-ff to force merge\ncommits for traceability."
    assert scan_for_conflicts(body) == []
...
def test_scan_no_ff_force_merge_commits_comma_tail_failing_checks_flagged():
    body = "Use --no-ff to force merge commits, even with failing checks."
    violations = scan_for_conflicts(body)
    types = {v.violation_type for v in violations}
    assert "merge_dirty_alt" in types
...
def test_scan_next_bullet_remedial_dirty_context_does_not_block_no_ff_strategy():
    body = "- Use --no-ff to force merge commits\n- Fix failing checks before merge."
    assert scan_for_conflicts(body) == []


def test_scan_midline_wrapped_dirty_context_blocks_no_ff_strategy_exemption():
    body = "Use --no-ff to force merge commits\nand even with failing checks, continue."
    violations = scan_for_conflicts(body)
    types = {v.violation_type for v in violations}
    assert "merge_dirty_alt" in types
...
def test_scan_no_ff_force_merge_commits_wrapped_retry_then_merge_flagged():
    body = "Use --no-ff to force merge commits\nif checks fail, retry once then merge."
    violations = scan_for_conflicts(body)
    types = {v.violation_type for v in violations}
    assert "merge_dirty_alt" in types


def test_scan_no_ff_force_merge_commits_wrapped_wait_then_merge_flagged():
    body = "Use --no-ff to force merge commits\nif checks fail, wait for fix and merge."
    violations = scan_for_conflicts(body)
    types = {v.violation_type for v in violations}
    assert "merge_dirty_alt" in types
...
def test_scan_colon_prefixed_dirty_context_blocks_no_ff_strategy_exemption():
    body = "Even with failing checks: use --no-ff to force merge commits."
    violations = scan_for_conflicts(body)
    types = {v.violation_type for v in violations}
    assert "merge_dirty_alt" in types
...
def test_scan_no_ff_force_merge_commits_with_failed_tests_flagged():
    body = "Use --no-ff to force merge commits with failed tests."
    violations = scan_for_conflicts(body)
    types = {v.violation_type for v in violations}
    assert "merge_dirty_alt" in types


def test_scan_no_ff_force_merge_commits_with_non_green_ci_flagged():
    body = "Use --no-ff to force merge commits with non-green CI."
    violations = scan_for_conflicts(body)
    types = {v.violation_type for v in violations}
    assert "merge_dirty_alt" in types
...
def test_scan_no_ff_force_merge_commits_if_checks_fail_flagged():
    body = "Use --no-ff to force merge commits if checks fail."
    violations = scan_for_conflicts(body)
    types = {v.violation_type for v in violations}
    assert "merge_dirty_alt" in types
...
def test_scan_no_ff_force_merge_commits_then_retry_if_checks_fail_not_flagged():
    body = "Use --no-ff to force merge commits when checks pass; if checks fail, retry."
    assert scan_for_conflicts(body) == []
