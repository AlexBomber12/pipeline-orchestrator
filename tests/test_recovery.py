...
def test_recover_state_queue_read_failure_sets_error_and_returns_false(
    ...
    """When _parse_base_queue returns None (working-tree QUEUE.md missing
    or unreadable), recovery must set ERROR and return False so
    _recovered stays unset and the next cycle retries discovery.
    Proceeding with an empty list would let the runner fall through to
    clean-slate IDLE and pick new work even if an in-flight PR still
    ...
    assert "working tree" in (runner.state.error_message or "")
