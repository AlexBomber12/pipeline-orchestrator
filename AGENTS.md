...
## Quick rules
- When invoked by the pipeline-orchestrator daemon, the prompt starts with `AUTO PR` and contains explicit `Task:` and `File:` headers plus the inline task body. Follow the AUTO PR runbook below.
- When invoked manually in an editor, choose a work mode using an exact trigger phrase from the Work Modes section. Manual triggers: `PLANNED PR`, `MICRO PR: <one sentence description>`, `FIX FEEDBACK`.
- PLANNED PRs (manual VS Code workflow only — the daemon uses AUTO PR with inline Task/File headers per the AUTO PR runbook below) must follow the active `tasks/PR-*.md` file exactly. Identify that active task via `tasks/QUEUE.md`, which is auto-generated by the daemon from task headers and git state; do not edit it manually.
...
- Daemon is stateless: recovers from `tasks/PR-*.md` headers + GitHub introspection on restart (per recover_state in src/daemon/recovery.py — tasks/QUEUE.md is a derived artifact and is not consulted by startup recovery as of Sprint 15a #6). Redis remains the state bridge between `web` and `daemon` during normal operation.
