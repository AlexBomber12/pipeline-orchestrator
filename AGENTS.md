...
- PLANNED PRs must follow the active `tasks/PR-*.md` file exactly. `tasks/QUEUE.md` is auto-generated by the daemon from task headers and git state; do not edit it manually.
...
Meaning:
- `PLANNED PR`: the default mode. Work strictly from the active `tasks/PR-*.md` file assigned by the daemon.
...
- Daemon is stateless: recovers from Redis + GitHub introspection on restart.
...
## PLANNED PR runbook (daemon-assigned)

### Rules
- Preflight: `git status --porcelain` must be empty. If not, stop and list dirty files.
- Use the daemon-provided `PR_ID` and `TASK_FILE`. Do not guess or select a different task locally.
...
### Checklist
- [ ] Preflight clean
- [ ] Recorded daemon-assigned `PR_ID` and `TASK_FILE`
- [ ] Read `TASK_FILE`
