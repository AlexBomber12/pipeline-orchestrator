...
| Sprint 15b | **First phase: architecture decision (no code, ~1 chat session) — recover Status field design from prior sessions, decide subset of statuses in PR-*.md vs Redis (avoid 1000 commits per micro-state change), decide reconciliation rules, decide PR-275 cross-repo intent permanently deprecated vs LLM-classification retry; second phase: Polish bucket + Tier 1 guardrails** (event log badges + time-ago alignment, per-repo coder readonly placement, theme toggle to Settings, global corner spinner, limit badges unified format, repo card buttons fixed-position layout, OBS-BT cross-repo task detection — pivot to PR-276/PR-277 stdout+diff layers per OBS-CR, OBS-BU Tier 1 guardrails framework, AGENTS-SCAN silence MICRO PR drafted but not shipped per OBS-CU, Status field implementation per OBS-CS) | Queued (Phase 1 = decision, no code) | Phase 1: ~1 chat session no PR. Phase 2: 7-8 PRs, ~12-15 daemon-hours |
...
- OBS-CS added — Status field absent from PR-*.md frontmatter (TaskHeader missing status field). Operator pain: only way to mark task as cancelled or blocked is delete file from disk. Architectural intent from prior sessions: file-level statuses subset of Redis statuses (avoid 1000 commits per micro-state change). Sprint 15b Phase 1 decision-making scope. Estimate after architecture clarity: 1-2 PRs ~4-6 daemon-hours.
