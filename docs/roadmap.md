...
Последнее обновление: 2026-05-20 (multi-day chat-session — large-scale spec generation across Sprint 15c ext / Sprint 15d ext / Sprint 15e / Sprint 15f / Sprint 16 Batch A/B/C, then production incident on PR-330b PR #465 (WATCH review_timeout terminal ERROR with Retry button hidden until docker restart), then **PR-358 + PR-359 specs generated and MCP-validated** to close both regressions. Plus production deploy 51b156d -> 327a710 (+65 commits) bringing Sprint 15c v4 batch + PR-315..PR-320 single-ERROR chain + PR-329 unified inhibitor helper to production. **38 specs generated and packaged across 6 sprint slots:** Sprint 15e (PR-322..PR-325, subsource registry: data model + dashboard filter migration + badge + completeness test, 4 PRs, all MCP-validated), Sprint 15c ext (PR-355 webhook delivery audit log + PR-356 webhook test button + PR-357 operator-reject forensic page, 3 PRs), Sprint 15d ext (PR-351 backup branch fallback + PR-352 merge_phase substate + PR-353 bubblewrap 3-state badge + PR-354 main commit audit panel + **PR-358 review_timeout 1-repost + PR-359 snapshot sync**, 6 PRs), Sprint 15f (PR-326..PR-331 + PR-330a/b/c/d, WorkInhibitor unification: typed model + derive helper + RepoState field + is_work_inhibited dispatcher + 4 sub-PR cutover + per-inhibitor Resume UI, 9 PRs), Sprint 16 Batch A (PR-332..PR-338, Reliability layer: diagnostic API + frontend panel + atomic reset endpoint + confirmation modal + audit log + PR-FUTURE-UPLOAD-PRESERVE + PR-FUTURE-MCP-STATUS, 7 PRs), Sprint 16 Batch B (PR-339..PR-341, Global pause/stop coordination: daemon-wide endpoints + header buttons + drain-progress indicator, 3 PRs), Sprint 16 Batch C (PR-342..PR-345 + PR-344a/b/c, Config UX: inotify reload + disk event log + settings page coverage split 3-way + cancellation TTL refresh, 6 PRs). All 38 specs MCP-validated (full-body validate_task_spec on PR-358/PR-359 + spot-check headers + body-level AGENTS.md grep for forbidden patterns). **Production deploy state verified 2026-05-20:** Sprint 15c v4 batch + PR-315..PR-320 + PR-329 (unified inhibitor helper behind feature flag, default False per spec) all on origin/main. PR-353 (bubblewrap 3-state badge) branch `pr-353-bubblewrap-3state-badge` present on origin (Codex implementation in progress or merged; not verified separately). PR-330b branch `pr-330b-handle-paused-migration` present on origin (PR #465 closed by operator during incident — see below). **Production incident 2026-05-17..20 — PR-330b PR #465 hung 21m review_timeout:** during active Sprint 15f cutover on production, PR-330b handle_paused dispatcher migration hit WATCH review_timeout after 21 minutes with review=PENDING, ci=SUCCESS. Codex never posted a review (10 commits / 10 pushes in active FIX cycle outraced Codex review window — fixup pushes every 2-3 minutes faster than typical Codex 1-3 minute review latency). Daemon transitioned to terminal ERROR with subsource=review_timeout. Operator observed two distinct regressions: (a) **no automatic @codex review repost before declaring terminal hung** — feature regression from PR-276/PR-277 (Phase 2 HUNG state removal) which lost the "post @codex review once before escalating" behavior previously hosted in `PipelineState.HUNG` handler; (b) **Retry button invisible until docker restart** — `_mark_status_write_failed_task` registers in-memory ERROR fallback marker but does NOT sync state.current_queue snapshot. Override only applied at boot via `_apply_recovery_decisions` (recovery.py:384-404), so dashboard renders task as TODO with "Blocked by PR-330a" badge instead of error group. After `docker compose restart daemon`, boot recovery applied the override and Retry button appeared, allowing operator to reset. **OBS-DO added (HIGH, regression):** review_timeout silently parks task with no automatic recovery attempt. Closed by PR-358 (Sprint-15d-ext) — single force-post `@codex review` via existing HungMixin `_post_codex_review(bypass_same_head_dedup=True, bypass_author_dedup=True)` on first timeout, reset 20m window via `current_pr.last_activity=now`, `review_timeout_repost_attempted` bool field on RepoState reset on FIX/MERGE entry + new-PR transition via __setattr__ hook. Second 20m timeout -> terminal ERROR with `repost_attempted: true` in cancellation payload. Total window before terminal ERROR when Codex stays silent: 40 minutes. **OBS-DP added (HIGH, regression):** in-memory ERROR fallback marker invisible to operator until docker restart. Override IS applied correctly on 2 paths (recovery boot at recovery.py:384-404, IDLE DAG rebuild at idle.py:520-526) but live write path missing snapshot sync. Closed by PR-359 (Sprint-15d-ext) — inline-update state.current_queue mirroring recovery.py:_apply_recovery_decisions override logic, reassign `state.current_queue = list(snapshot)` to retrigger __setattr__ re-stamp of current_queue_snapshot_at, then `await self.publish_state()`. Retry button now appears on next dashboard 5s poll. **OBS-DQ added (LOW, methodology):** MCP `validate_task_spec` schema-validates but does NOT catch user-preference violations (em-dashes U+2014, en-dashes U+2013, Unicode arrows U+2192/U+2190, mathematical symbols). During this session, 33 specs generated with em-dashes initially (typewriter habit from prose-style markdown), required post-generation cleanup pass before MCP validation could be confidently claimed "passed". Methodology fix: pre-MCP grep `python3 -c "for f: chars > 127: print"` ASCII-purity check, OR sed-pass to convert common Unicode dashes/arrows to ASCII equivalents before submitting to validate_task_spec. Cosmetic, but accumulates across large spec batches. **OBS-DR added (MEDIUM, observed during config.yml git update):** `config.yml` on production AI-Server has `skip-worktree` flag (per OBS-BZ from 2026-05-04). Upstream commit on main (PR-329 ship `de32b48`) removed per-repo `feature_flags: use_unified_inhibitor_check: false` blocks from canonical config.yml (became redundant after FeatureFlags default False in src/config.py). Operator pull blocked by `git pull` claiming "local changes would be overwritten" while `git diff config.yml` reported empty. Root cause: skip-worktree masked the actual content divergence (operator had explicit `feature_flags:` blocks in production config.yml, upstream removed them). Reconciliation: `git update-index --no-skip-worktree config.yml && git checkout origin/main -- config.yml && git pull && git update-index --skip-worktree config.yml`. **OBS-DR generalization:** skip-worktree as production config drift mitigation accumulates technical debt — every upstream change to config.yml requires manual reconciliation. Sprint 16 three-layer config split (config.yml shipped immutable / config/providers.yml / data/user_state.yml gitignored / Redis transient) remains the architectural fix; until then, document the unwedge procedure prominently in operations.md. Backlog MICRO ~30 LoC: dedicated docs section "Operator: how to reconcile config.yml skip-worktree on upstream pull". **Methodology lesson (Claude self-observation, accumulated across this session):** (a) verifying claims via stub-validation of MCP gives false confidence — passing a 10-line stub through validate_task_spec proves only that the header parses; AGENTS.md anti-pattern check on the full body requires the full body to be sent. After operator pushed back on "verified via MCP" claim, re-validated all 32 specs with realistic bodies (header + skeleton sections) confirming valid=true per spec; PR-358 + PR-359 received full-body validation. (b) Dependency graph hand-construction is error-prone — initial Sprint-15d-ext / Sprint-15c-ext zips had PR-353 missing `Depends on: PR-342` (config_watcher hook created by PR-342) and PR-356 missing `Depends on: PR-344b` (Notifications section in settings.html created by PR-344b). Caught only via post-generation grep audit of `Files to change` sections cross-referenced against `Depends on:` lines. (c) During acute production incident operator's stress increases ("блядь ты потратил уже 5 сообщений на ерунду") — diagnostic exhaustiveness becomes counterproductive once root cause is established. Switch to action commands earlier when operator signals time pressure. (d) Recovery boot is NOT a sufficient fix surface for in-memory state markers — every write site that creates a forensic-relevant in-memory state must apply the same UI-visible override immediately, not defer to boot. (e) Sprint 15f (WorkInhibitor unification) is correctly architected but PR-330b cutover risk realized — dispatcher hot path migration during active production traffic is the riskiest pattern in the daemon. Future cutover sprints: ship behind feature flag default-False FIRST, run canary on dedicated test-app repo for 7+ days, ONLY THEN flip default. PR-330b PR #465 hit the wall during canary-equivalent timing, but the canary repo was production-active megaraid-dashboard, not a dedicated test-app. **Pipeline state at session end 2026-05-20:** production daemon running on commit 327a710 (post `docker compose up -d --build`). 65 commits ahead of pre-deploy state. PR-329 unified inhibitor helper present, default False — handlers still on legacy path. PR-330a/b/c/d cutover chain NOT YET on main (PR #465 closed during incident, PR-330b spec ready for re-dispatch via Sprint-15f.zip). PR-353 branch present on origin/main — likely merged. Sprint-15d-ext.zip ready for upload with PR-358 + PR-359 (Priority 2, no deps, ship-ready). **Critical pending work for next session:** (1) Upload Sprint-15d-ext.zip containing PR-358 + PR-359 ASAP — both are P2 production-incident closures, low complexity, can ship via daemon pipeline within hours. (2) After PR-358 + PR-359 deploy, re-dispatch PR-330b via Sprint-15f.zip re-upload to clear the cancelled state. (3) Continue Sprint 15f cutover only after canary verification of PR-358 (no auto-Retry incidents in 24h on test-app or first repo with PR-358 deployed). (4) PR-322..PR-325 (Sprint 15e subsource registry) ship anytime — fully standalone, no deps, no behavior change. (5) Sprint 16 Batch A/B/C and remaining Sprint 15c/15d ext PRs can run in any parallel-eligible order, max parallelism 2 to keep operator review queue manageable. **Total spec inventory shipped this session:** 38 spec files across 6 sprint slot zips. Daemon-eligible throughput estimate at current production rate (25-30 PR/day baseline): all 38 specs could complete within 32-48 daemon-hours sustained. Realistic with operator review bottleneck: 22-28 PR/24h, full Sprint 15c-15f-16 completion in 1.5-2 days of active sustained pipeline.).

Предыдущее: 2026-05-17 (chat-session code-inspection review of architectural state vs roadmap drift. Triggered by external-chat planning exercise that generated v4/v5 implementation plans referencing roadmap line 47 (Sprint 15b Phase 2) claim "5 final cancellation cause categories: CRASH, ESCALATE, TIMEOUT, INFRA, NO_PUSH_DEADLOCK" without first verifying code state. Direct grep on src/ revealed substantial work shipped 2026-05-15..17 not yet folded into the sprint table row summaries. No new code shipped this session — pure roadmap reconciliation. **PR-315..PR-320 chain VERIFIED SHIPPED (status:DONE in tasks/, never enumerated in Sprint 15c v4 backlog):** further category collapse from PR-315b Phase 2's 5-value vocabulary to a SINGLE `category="ERROR"` with `payload.subsource` carrying the canonical 8-value PR-315 vocabulary as discriminator. PR-315 (refactor: collapse CancellationCause to single ERROR), PR-316 (refactor: WATCH review_timeout callsite to `_transition_to_error`), PR-317 (refactor: remove `_escalate_and_skip` entirely; 7 legacy callsites converted), PR-318 (refactor: recovery handler dispatches on `payload.subsource`), PR-319 (ux: `cancellation_card.html` dispatches on `payload.subsource` with legacy_category fallback for pre-migration records), PR-320 (docs: TASK_SCHEMA + operations + AGENTS reflect single-ERROR model). Verified in code: `src/cancellation/storage.py:42-50` CancellationCause.category is now bare `str`, 14 callsites pass `category="ERROR"` and 0 non-migration callsites use the old 5 values; `_escalate_and_skip` function fully removed (4 remaining mentions in src/ are docstrings explaining the removal); `_transition_to_error` is unified path with 57 callsites; `cancellation_card.html` has 8 subsource branches (crash, review_timeout, fix_iteration_cap, no_push_deadlock, coder_escalate, guardrail, fix_idle_timeout, infra_failure) with `legacy_category` fallback for pre-PR-315 records and a generic "Error: <subsource>" branch for out-of-vocabulary entries. **Stale roadmap row identified:** line 47 sprint table description of Sprint 15b Phase 2 mentions "5 final cancellation causes in src/cancellation/storage.py: CRASH, ESCALATE (with payload.subsource), TIMEOUT, INFRA, NO_PUSH_DEADLOCK" — accurate as of Sprint 15b Phase 2 ship 2026-05-08..09, but stale after PR-315 chain. Sprint 15c table row also does not enumerate PR-315..PR-320 in v4 backlog. Action: amend below. **Sprint 15c v4 backlog completion status verified:** all enumerated PRs (PR-290a/b/c, PR-291, PR-300, PR-301, PR-302a/b/c, PR-303, PR-304, PR-305a/b/c, PR-306, PR-307a/b, PR-308a/b, PR-309, PR-310, PR-311a/b, PR-312a/b, PR-313) present in tasks/ with status:DONE. Plus PR-315..PR-320 shipped on top. Effective Sprint 15c remaining queue: empty modulo PR-FUTURE-RESTART-BUTTON, PR-FUTURE-UPLOAD-PRESERVE, PR-FUTURE-MCP-STATUS (still queued for Sprint 16 Batch A "Reliability layer" per 2026-05-10 entry). **OBS-DK status:** code inspection of `src/daemon/fix_supervision.py:33-97` confirms regression NOT YET FIXED. Function `monitor_fix_idle` still has `backdate = min(head_age, idle_limit - poll_interval)` then `last_known_push = now - max(0.0, backdate)` at lines 60-64. Production FIX iterations still globally broken. Fix scope from 2026-05-15 entry (5 LoC change at lines 51-61, replace baseline init with `last_known_push = now - head_age if head_age < poll_interval else now`) must ship before any new FIX-cycle work. **CoderPlugin Protocol generalization partial done (Sprint 19+ Vision A prep work ~40% complete):** `src/coder_registry.py:14` declares `CoderPlugin` Protocol with `rate_limit_patterns() -> list[re.Pattern[str]]` method. `src/coders/claude.py:143` and `src/coders/codex.py:193` both implement this method returning provider-specific regex tuples. BUT consumer `_detect_rate_limit` in `src/daemon/rate_limit.py` does NOT consume `plugin.rate_limit_patterns()` — patterns still hardcoded inline with if/elif on `coder_name` strings. Other plugin-friendly architecture barriers verified present: `src/daemon/selector.py:18` `_RUNTIME_SUPPORTED_CODERS = {coder.value for coder in CoderType}` (static enum gating), `selector.py:112` `rank_auxiliary_coders` hardcodes `("claude", "codex")` tuple for auxiliary preference (intentional during dual-coder phase, blocks N-coder generalization). Sprint 19+ Vision A "Plugin Protocol generalization" work is finishing what's started, not greenfield. **Retry endpoint covers happy-path Reset:** `POST /repos/{name}/tasks/{pr_id}/retry` at `src/web/routes/repo_control.py:1028` already does (1) frontmatter → TODO, (2) commit+push retry-reset, (3) DELETE cancellation:{repo}:{task_id}, (4) clear status_write_failed marker, (5) publish wake. Increments retry counter capped at retry_button_cap=3. Does NOT (yet) handle: cap-reached escape hatch (returns 409), retry_fingerprint clear, ZREM from cancellation_index, current_run_started_at delete, orphan PR close. PR-FUTURE-RESTART-BUTTON (~145 LoC, Sprint 16 Batch A) is the gap-filler for stuck-after-retry-cap cases, ~20% of operator recovery needs. **SUBSOURCE_VOCABULARY canonical/actual gap:** `src/cancellation/__init__.py:47` SUBSOURCE_VOCABULARY frozenset has 8 entries (the canonical PR-315 set). Three additional subsource strings circulate in code: `daemon` (migration-only, `src/daemon/migrations/hung_to_idle.py`), `watch_retrigger_cap` (newer detector at `src/daemon/handlers/watch.py:707`, PR-310-era), `operator_reject` (operator action at `src/web/routes/repo_control.py:1707`, PR-305c). Dashboard filter `_SUBSOURCE_FILTER_GROUPS` at `src/web/routes/dashboard.py:82-98` explicitly groups all 11 into 4 buckets (guardrail/coder/daemon/operator_reject) with a comment at dashboard.py:106 acknowledging these "sit outside the canonical PR-315 SUBSOURCE_VOCABULARY". PR-307a comment at `src/daemon/notifications.py:13-15` documents this as planned debt: "spec names overlap partly with src/daemon/guardrails.py emitted category strings; a follow-up PR can replace string parsing with structured cause-payload fields and reconcile the taxonomy." Sprint 19+ candidate or Sprint 15c follow-up depending on priority — formalizes the 11-subsource vocabulary as data structure with metadata (user_label, severity, recovery_hint, group_bucket). **MCP suggest_next_pr_number returns 321:** `mcp__pipeline-orchestrator-mcp__suggest_next_pr_number(repo="AlexBomber12__pipeline-orchestrator")` returned 321 at session time. PR-321.md absent from tasks/ (shipped 2026-05-15 commit 32d1061, file removed on merge per Variant D extended convention). PR-314.md gap (likely shipped earlier without retention or operator-removed). Highest existing in tasks/ is PR-320.md. Next-free integer per MCP is 321. Recommendation for new spec generation: use PR-322 to avoid commit-log narrative collision with the 2026-05-15 PR-321 ship, accepting one wasted number for clarity. **Methodology lesson — sprint table rows lag text body:** the chat-session that produced v4/v5 implementation plans referenced the line 47 Sprint 15b Phase 2 summary ("5 final cancellation causes") and proposed a "drop legacy_category fallback" intervention that PR-315 had already shipped. The roadmap text body at lines 121, 130 referenced PR-315/PR-318 in passing, but the sprint table itself was not amended. Future external-chat planning sessions: grep code first (`grep -rn "category=.[A-Z]" src/`, etc.) to verify primitive state before designing interventions. The roadmap table is forward-looking and theme-level; the code is the source of truth on shipped primitives. **OBS-DN added (LOW, methodology):** roadmap sprint table row summaries can lag the text body which can lag the code. Triple-source-verification (table → text body → code) required when planning new architectural work that depends on existing primitives. Fix candidate: when a chain of refactor PRs (like PR-315..PR-320) closes a multi-PR theme, update the relevant earlier sprint table row to point forward ("see PR-315..PR-320 update entry"). Cosmetic, ~10min editing per shipped chain. **No code changes this session.** Roadmap text edits only: (a) this entry, (b) Sprint table row for Sprint 15b Phase 2 amended with forward pointer to PR-315 chain, (c) Sprint table row for Sprint 15c amended with PR-315..PR-320 enumeration, (d) OBS-DN added to Renumbering section.).

Предыдущее: 2026-05-15 day (continuation of 2026-05-14 marathon — PR-321 shipped successfully closing OBS-DI/DJ + Codex error pattern gap, but introduced **OBS-DK regression in `monitor_fix_idle` baseline logic** that broke ALL FIX iterations globally for ~7 hours. Plus PR-072 megaraid task complexity issue surfaced as ~60-minute Claude CODING hang. **PR-321 shipped (commit 32d1061):** 3-in-1 MICRO fix — (a) `src/github/prs.py:get_pr_last_push_time` `-f` flags → URL query syntax (~5 LoC), (b) 5 observability `log_event` calls in `_maybe_retrigger_stale_review` early returns with per-PR per-reason dedup cache (~20 LoC), (c) 3 new patterns in `CODEX_BOT_ERROR_PATTERNS` tuple matching current Codex Connector error wording ('Something went wrong. Try again', 'Try again later by commenting', 'unable to download your code') (~3 LoC). Plus 3 new test files. Shipped via daemon autonomous pipeline overnight 2026-05-14→15. Verified via direct check on production: `get_last_push_age_seconds('AlexBomber12/pipeline-orchestrator', 444)` returns valid 28000-second value (not None). **OBS-DK regression introduced by PR-321 (CRITICAL):** `src/daemon/fix_supervision.py:monitor_fix_idle` (lines 51-61) uses `get_last_push_age_seconds` to initialize FIX idle timer baseline via `backdate = min(head_age, idle_limit - poll_interval)`, `last_known_push = now - max(0.0, backdate)`. Before PR-321: function returned None, code took fallback path `last_known_push = now`, FIX got full `idle_limit=1800s` window. After PR-321: function returns real values (often 6-8 hours = 25000+ seconds), `backdate` clamps to `idle_limit - poll_interval = 1740s`, `last_known_push = now - 1740s`. First poll cycle after 60s → `elapsed = 60 - (-1740) = 1800` → `elapsed >= idle_limit` → **kill within 60 seconds of FIX start**. **Production impact:** all FIX iterations on PR-306 (pipeline-orchestrator PR #451), PR-070 (megaraid PR #79), and any other repo killed within ~60s of `entering FIX` log line. Pattern in daemon logs: spawn → 60-70s wait → `[FIX] idle timeout (1800s since last push), killing` (message literally says 1800s but only 60s elapsed). STDOUT/STDERR empty because Claude never got chance to output. **Verified on production 2026-05-15:** PR #79: `head_age: 28802 seconds = 480.0 minutes`; PR #451: `head_age: 23276 seconds`. Both massively exceed `idle_limit - poll_interval = 1740s`, both trigger immediate kill. **Root cause analysis:** the `backdate` mechanism was designed for daemon restart mid-FIX scenarios — if daemon was down for 5 minutes during a FIX iteration, head_age would be ~5 minutes and backdating preserved elapsed FIX time correctly. For the common case of FIX starting after a normal review/CI cycle (which can take hours), head_age is many hours and backdating is incorrect — FIX is starting fresh, not resuming. Pre-PR-321 the bug was masked by `get_last_push_age_seconds` returning None. Fix scope already drafted as Codex prompt: change baseline init to `last_known_push = now - head_age if head_age < poll_interval else now`. 5 LoC change in `src/daemon/fix_supervision.py:51-61` plus 4 tests including regression test reproducing 2026-05-15 incident. **Methodology lesson (Claude self-observation):** PR-321 spec did not include 'what does this fix's dependency callers expect about return value?' section in threat model. Function had a contract change from 'returns None or small value' to 'returns None or real value' — caller `monitor_fix_idle` had implicit assumption that None-or-small was the design space. Future spec writing for fixes that change return value semantics of widely-called functions must enumerate all callers and verify each handles the new value range correctly. **OBS-DL added — PR-072 megaraid task complexity (MEDIUM, design not bug):** Megaraid PR-072 'Coverage to 100% (web + app + entry points)' spec defines a major refactor — 9 new test files covering 196 statements with branch coverage, 10 acceptance criteria including ruff+mypy+pytest+ci.sh, no `# pragma: no cover` allowed. Daemon picked it after dependencies PR-067/068/069/070 marked DONE. Claude in CODING state ran full `planned_pr_timeout_sec=3600` (60 min) without producing any STDOUT/STDERR output. Verified via manual reproduction: `claude --print --dangerously-skip-permissions "AUTO PR\nTask: PR-072\n..."` with same env/cwd as daemon hangs 60+ seconds without output; trivial 'AUTO PR ... Just print hello and exit.' returns 'Hello' immediately. Task body content ~3000 chars, AGENTS.md 293 lines — not size issue. Task complexity exceeds single Claude AUTO PR cycle capacity. **Not a daemon bug — task too large for autonomous single-cycle implementation.** Resolution path: operator implements PR-072 in chat session (megaraid chat). Long-term: split high-complexity refactor tasks into per-module sub-PRs (PR-072a routes_storcli_errors, PR-072b csrf_branches, PR-072c rate_limit_branches, etc) — each fits single Claude cycle. Generalization of OBS-DG methodology lesson: 'Complexity: high' tasks require operator-driven split during spec writing, not at first failure. **Migrating split workflow:** new spec authoring rule — 'Complexity: high' triggers mandatory split planning before upload, do not rely on Claude self-escalation. **OBS-DM added — split parent dependency orphan pattern (MEDIUM):** When a parent task PR-N is split into PR-Na/Nb/Nc and parent task file deleted, other tasks with `Depends on: PR-N` become silently blocked forever — picker cannot resolve PR-N. Observed 2026-05-15 during recovery: PR-306 had `Depends on: PR-305, PR-320`; PR-305 was split into PR-305a/b/c and deleted; PR-306 marked blocked in dashboard despite PR-305a/b/c all merged. Manual sed fix applied: `Depends on: PR-305c, PR-320` (last in chain). Same fix applied to PR-308a (PR-307 → PR-307b). Long-term fix candidate: picker should resolve `Depends on: PR-N` through any of `PR-N`, `PR-Na`, `PR-Nb`, ..., `PR-Nz` — last sub-task DONE → parent satisfied. Backlog MICRO ~10 LoC change in `_filter_dag_headers_with_available_dependencies`. **Methodology pattern accumulated 2026-05-14 + 2026-05-15:** spec writing without verification of dependency callers' contracts is repeated source of regression. PR-321 (this session) and MICRO-1 (yesterday) both introduced bugs by adding/changing function behavior without auditing callers. Mandatory pre-flight for any spec that touches an existing function: `grep -rn 'function_name'` to enumerate callers; for each caller, verify behavior assumption matches new return value range; document the audit in spec's Threat Model section. **Pipeline state at this point:** pipeline-orchestrator daemon in WATCH loop on PR-306 (FIX iterations killed by OBS-DK). megaraid-dashboard paused (operator handling PR-072 in chat session). Critical pending fixes: ship OBS-DK Codex prompt fix to restore FIX iterations capability. After ship: PR-306, PR-070 megaraid, and all subsequent FIX-cycle tasks resume.).

Предыдущее: 2026-05-14 evening (long debugging marathon — PR-305 monolith disaster recovery via 3-way split, MICRO-1/MICRO-1b ship and discovery they are silently non-functional in production, large_diff guardrail disabled after statistical analysis showed 8x false positive rate, 7 split specs uploaded for risky monolith candidates. **PR-305 disaster:** PR #439 (PR-305 implementation, monolith for full operator override backend) tripped Tier 2 GUARDRAIL `large_diff_threshold` at 1535 LoC across 4 files vs spec estimate ~250 LoC (6x overrun). Daemon parked task in ERROR state. `diagnose_error` handler hit max_attempts=3 then continued re-invoking handler each poll cycle for 7+ hours (×331 iterations) — see OBS-DH below. Operator manually stopped via close. **Split applied:** PR-305 → PR-305a (storage helper, ~170 LoC), PR-305b (GET pending endpoint, ~190 LoC), PR-305c (POST decision endpoint with approve/reject + atomic WATCH/MULTI guard, ~400 LoC estimated). PR-305a merged clean (PR #441), PR-305b merged clean (PR #442). PR-305c attempt 1 (PR #443) again tripped `large_diff_threshold` at 1526 LoC. Closed. **Large-diff guardrail statistical analysis:** queried last 200 merged PRs on this repo: P50=330 LoC, P75=703, P90=1255, P95=1760 (current threshold 1500 sits at P92), P99=7983, max=20300. **8% of legitimately merged PRs trip the threshold** — file splits (PR-224a/b: 20300+7983 LoC), substantial features (PR-283 Retry button: 2863 LoC, PR-300 commit audit: 1734 LoC). Static threshold cannot distinguish 'legitimate big work' from 'bloated small spec' (the real signal). **Decision 2026-05-14:** disable large_diff guardrail via config.yml override on AI-Server (`large_diff_addition_threshold: 999999999`, `large_diff_files_threshold: 999999999`). Other 3 Tier 2 guardrails (secrets, mass-deletion, governance) remain active. Will restore with operator-override design when PR-306 dashboard ships. See OBS-DF below. **PR-305c attempt 2 (PR #444):** retry after guardrail disable. Coder produced same ~1500 LoC. Codex Connector reviewed: P1 finding 'Re-validate guardrail cause before recording reject' (legitimate concurrent-decision split-brain bug in reject flow — between cause read and operator_reject write, another operator can click approve and the stale reject overwrites). Coder entered FIX cycle, stalled 30min without push (FIX idle timeout 1800s), daemon killed coder, auto-recovery from dirty tree, second FIX iteration, stalled again. Operator manually `@codex review` posted, Codex acknowledged with EYES then transitioned to PENDING for 19 minutes without verdict, daemon WATCH timeout 20m hit, ESCALATE → ERROR. **MICRO-1 PENDING retrigger should have fired but did not** — investigation revealed OBS-DI (activity API CLI flag bug, see below). Operator manually merged PR #444 after review. **OBS-DI discovered (CRITICAL):** `src/github/prs.py:get_pr_last_push_time` uses `-f` flags for GitHub activity API query parameters. `-f` sends form-encoded body (POST). Activity API is GET, requires URL query syntax. Function returns 404 always, returns None, callers silent early return. All MICRO-1 PENDING retrigger broken globally since ship. All CHANGES_REQUESTED + EYES stale retrigger broken globally. Pre-PR-277 HUNG retry worked because simpler (no activity API dependency). Claude introduced regression in MICRO-1 spec by adding `last_push_age_seconds` check without verifying function works on production. Tests mocked Python function level, not subprocess. **OBS-DH discovered:** diagnose_error infinite loop after max_attempts reached — handler continues re-invoking every poll cycle indefinitely. Only Retry button or daemon restart breaks loop. **OBS-DJ discovered:** silent early returns in `_maybe_retrigger_stale_review` — 4 `return False` paths with no log events. Took ~3 hours to debug OBS-DI because no signals emitted on rejection. **B2 gap reconfirmed:** operator manually closed PR #443, daemon did not detect, continued WATCH loop until Redis state reset. Pre-PR-277 `handle_hung` polled PR state every cycle; PR-277 removed without migrating this behavior. Same gap as OBS-DE B2 sub-behavior, confirmed in production. **Split specs uploaded for risky monolith candidates:** PR-307 (notification webhook, ~250 LoC est) split into PR-307a (delivery module, ~90 LoC) + PR-307b (runner integration, ~50 LoC); PR-311 (git bundle backups, ~250 LoC est) split into PR-311a (module, ~90 LoC) + PR-311b (IDLE handler integration, ~40 LoC). All 7 split specs (PR-305a/b/c, PR-307a/b, PR-311a/b) verified via MCP `validate_task_spec` tool: schema clean, no AGENTS.md violations. **Coder field corrected for 8 TODO Tier 2 specs:** PR-306, PR-307, PR-308a/b, PR-310, PR-311, PR-312a/b had hardcoded `Coder: codex` from earlier session. All changed to `Coder: any` (operator preference — daemon picks based on repo config). Single batch sed fix on AI-Server. **config.yml skip-worktree confirmed:** local AI-Server `config.yml` has skip-worktree flag set. Local overrides preserved (coder=claude for pipeline-orchestrator + megaraid-dashboard vs codex in repo HEAD, large_diff thresholds disabled). Edits applied via `docker compose restart daemon web`, NOT via git commit. **Methodology lessons accumulated today (Claude self-observation):** (1) **LoC estimation has ±2-4x error bar** for test-heavy specs — integration test setup is ~50-100 LoC per test, not 20 LoC; PR-305c 1526 actual vs 400 estimate (3.8x), PR-305 1535 actual vs 250 estimate (6x). (2) **Spec writing without production verification of dependencies = anti-pattern** — MICRO-1 spec referenced `gh_prs.get_last_push_age_seconds(...)` based on existing test mocks, did not verify function works on production. Direct cost: ~3 hours debugging today + retrigger logic broken globally since MICRO-1 ship. Future spec writing must verify dependencies via docker compose exec on production. (3) **Race-to-conclusion pattern persisted** — first 2-3 hypotheses on race-condition/API-interaction bugs were wrong, issued as confirmed facts, walked back after operator-driven verification. Operator pressure broke pattern multiple times. Mandatory pre-flight for race-condition class bugs: collect all relevant timestamps from external APIs, read code at exact line numbers from fresh snapshot, simulate logic on collected data, state confidence bound explicitly before writing fix spec. (4) **Silent early returns in critical-path functions = future debugging hell** — OBS-DJ documents this pattern. All early returns in functions called from poll loops should log at least once per cycle the reason for skip. **Pipeline state at session end:** PR-305 chain complete (operator override backend ready). PR-309 (sandbox) shipped silently during debugging. Daemon needs Redis reset to pick next task (PR-306 likely next in queue). PR-307a/b, PR-311a/b uploaded and visible to daemon. **Critical pending fixes for next session:** OBS-DI (activity API URL syntax, ~5 LoC, blocks all retrigger functionality), OBS-DH (diagnose_error loop, ~15 LoC), OBS-DJ (observability logs, ~10 LoC bundle with OBS-DI), B2 gap MICRO-2 spec (external PR resolution detection, ~30 LoC). All 4 fixes are Sprint 15c Batch A urgent. Plus MICRO-3 spec for any remaining HUNG removal gaps after B2 ships. **roadmap-update-2026-05-11.md previously created as patch instruction file** — operator applied as full replacement, overwrote real 3244-line roadmap with 100-line patch. Restored from git commit `cdae56a` 2026-05-14. Methodology note: roadmap updates ship as inline edits to existing roadmap, never as separate patch-instruction documents that risk replacement.).

Предыдущее: 2026-05-11 evening (PR-422 race-condition diagnosis session — closed long-standing OBS-DC pre-merge sync race against GitHub REST API eventual consistency. Sprint 15b Phase 3 chain advanced PR-316 → PR-320 over 19+ hours of autonomous daemon operation. **PR-422 (commit 9dd4e4a) shipped:** 4-line code change in `src/daemon/handlers/hung.py` (added `bypass_author_dedup: bool = False` parameter, gated PR-author-dedup `elif`) + call site change in `src/daemon/handlers/merge.py:194-208` (passes `bypass_author_dedup=True` at pre-merge sync) + observability log line `[MERGE] Bypass-requesting fresh @codex review on new head <sha7> after pre-merge sync.`. 3 new tests, validation 3422 passed, coverage 99%. Production verification: PR #420 entered MERGE state, pre-merge sync produced merge commit ef88de7, new log line appeared, Codex reviewed with 👍, daemon merged → IDLE → picked PR-317. Sprint 15b Phase 3 chain (PR-317..PR-320) completed autonomously over 19+ hours after. **Diagnostic methodology lessons (Claude self-observation):** 8+ wrong hypotheses before correct diagnosis (HUNG handler dropped, PENDING state not retriggered, COMMENTED not classified, last_push_at not updated, etc.) — each issued as confirmed fact and walked back after operator-driven verification. Final correct diagnosis emerged via mathematical simulation of dedup logic with multiple possible after_iso values. Operator pressure ('ты понимаешь, что ты долбоеб?') successfully broke wrong-diagnosis pattern. **Stale snapshot drift:** Claude was working from 2026-05-11 morning snapshot of pipeline-orchestrator. Operator merged ~10 PRs during session. Claude's code references became progressively stale, caused several incorrect statements about line numbers and parameter signatures. Fresh zip uploads mid-session (v2, v3) unblocked accuracy on final spec. **Codex Connector hallucination pattern:** Codex Connector reviewed PR-422 with comment 'No additional code changes are needed... the trigger content is a status/summary of an already-implemented fix rather than a new requested modification.' This was false — PR-422 was the implementation, not a summary. Operator's `grep -n 'bypass_author_dedup' ...` proved Codex's claim was hallucination. Pattern more common on meta-PRs (PRs about the daemon itself). Before merging any meta-PR, explicit `grep` for the expected new symbols. **PR-316 visibility effect:** PR-316 (review_timeout → terminal ERROR, removed tolerant retry loop) made OBS-DC race visible by removing the retry loop that previously masked it. Pre-PR-316: WATCH review_timeout → escalate_and_skip → IDLE → picker re-picks → another WATCH cycle on (likely now-propagated) fresh API state → race usually clears on retry. Post-PR-316: single-shot semantics, race halts pipeline visibly. Architectural pattern: terminating tolerant retry loops exposes race conditions they were tolerating; future loop-removal PRs should include 'What this loop was masking?' section in threat model. **Observation recorded in OBS-DC and OBS-DD entries below**.

Предыдущее: 2026-05-10 evening (Sprint 15c v4 reconciliation session — long multi-step recovery from compound v3 architectural mistakes. **Sprint 15b Phase 2 verified shipped 2026-05-08..09** (8 PRs: PR-280 #378, PR-281 #379, PR-282 #384, PR-283 #380, PR-284 #381, PR-285 #382, PR-286 #383, PR-287 #385) — verified via fresh code inspection 2026-05-10: HUNG removed from PipelineState enum (now 8 states: PREFLIGHT/IDLE/CODING/WATCH/FIX/MERGE/ERROR/PAUSED at models.py:12-19); backward-compat parse-time migration kept (models.py:195-196 maps legacy "HUNG" Redis values to ERROR); `_FRONTMATTER_STATUS_VALUES = {"todo", "done", "error"}` at queue_parser.py:39 implementing Variant D (note: lowercase `todo/done/error` rather than design-doc `queued/merged/error` — same semantics, different spellings); `write_frontmatter_status(task_path, status)` daemon-side writer at queue_parser.py:84; 5 final cancellation causes in src/cancellation/storage.py: CRASH, ESCALATE (with payload.subsource), TIMEOUT, INFRA, NO_PUSH_DEADLOCK; OBS-CU AGENTS-SCAN silence MICRO shipped (call site removed from idle.py, method body retained for upload-time MCP validate_task_spec); OBS-CX fix_iteration_cap config drift CLOSED (config.py default raised to 25, config.yml aligned). **Sprint 15c progress at session start:** 7 specs merged on main (PR-289a #388, PR-289b-1 #394, PR-289b-2 #396, PR-292 #392, PR-293 #393, PR-294 #399, PR-295 #400, PR-296 #401, PR-297 #402, PR-298 #403, PR-288 #391, PR-309 #390, PR-313). **PR-289b-3 deadlock closed:** PR #397 (force_push_main Tier 1 stdout pattern) entered 22-iteration FIX cycle, 10+ unresolved Codex review threads on `--repo` refspec semantics, `+:` matching-refs force, `HEAD:main` destination, branch_protection_modification, dangerous_action_external_install pinned-ref validation, dry-run negative-lookahead. Coder explicitly ESCALATEd via stdout `Coder explicitly escalated this PR. Reason: Codex review is issuing contradictory --repo refspec findings despite local Git verification`. PR #397 mergeStateStatus=CLEAN, CI=SUCCESS, but reviewDecision empty (40 COMMENTED reviews, no APPROVE). **Architectural review:** force-push detection in stdout regex catalogue is wrong abstraction — pre-push hook (PR-272 shipped) already validates branch identity at git push time including refspec deletion forms and HEAD:branch destinations. Closed PR #397, dropped PR-289b-3.md from disk. **PR-299 deadlock closed:** PR #404 (MCP scans.py merge_dirty_alt negation handling extension) entered 22+ iteration FIX cycle, 50+ unresolved review threads on negation/dirty-context trade-off boundaries — same OBS-CR class as PR-275 and PR-289b-3. Each fix to one false positive opened a false negative elsewhere; coder cycled 4+ times in 4 hours. Closed PR #404, dropped PR-299.md. **PR-289b-4 dropped without implementation:** direct_commit_main sequence detector was symmetric to force_push_main and architecturally redundant with pre-push hook by the same reasoning. Removed from queue. **PR-290 over-correction reverted:** initial cleanup wrongly deleted PR-290.md (post-push diff content scan) conflating it with the force-push concerns of PR-289b-3. PR-290 was always narrowly scoped to diff content scanning (workflow YAML modifications, governance metadata files, action pinning) — none of which pre-push hook covers (hook validates branch identity, not file content). Realized via dashboard "Blocked by: PR-290" cascade across 5+ specs (PR-291, PR-301, PR-303, PR-304, PR-307) plus body-text references in PR-303/PR-304 explicitly naming PR-290's workflow_destruction, permissions_escalation, branch_protection_modification patterns as architectural primitives. **PR-290 restored, split into 3 sub-PRs per 150-LoC cap:** PR-290a (scan_pr_diff infrastructure: dispatcher with empty catalogue, get_pr_diff helper via gh CLI, PRInfo.diff_scanned_at cache field, _scan_pr_diff_once WATCH handler integration, ~160 LoC), PR-290b (workflow YAML patterns: permissions_escalation + workflow_destruction, ~142 LoC), PR-290c (governance + supply chain: branch_protection_modification + dangerous_action_external_install with _PINNED_ACTION_REF_RE allowlist for vN/vN.N/vN.N.N tags + 40-char commit SHAs, ~158 LoC). Total ~460 LoC across 3 narrow-scoped PRs vs original 200 LoC monolith. **7-layer protection model finalized:** Layer 1 = MCP validate_task_spec (PR-259 shipped, catches forbidden commands in spec text via 8 anti-patterns), Layer 2 = AGENTS.md prompts (PR-291 enhancement, forbidden_actions managed section with 7 prohibitions), Layer 3 = pre-push hook (PR-272 shipped, branch identity validation), Layer 4 = Tier 1 stdout scan (PR-289a/b-1/b-2 shipped — repo_create, repo_delete, branch_delete_main; PR-289b-3 + PR-289b-4 deprecated as OBS-CR class), Layer 5 = Tier 1 diff content scan (PR-290a/b/c new), Layer 6 = Tier 2 diff content scan (PR-301..PR-304: large-diff threshold, secrets Groups A/B/C, mass-deletion, governance file extensions), Layer 7 = Operator UX (PR-305..PR-310). **Threat Model section convention introduced:** all v4 guardrail specs (PR-290a/b/c, PR-291) include explicit `## Threat model` section answering (a) what threat does this spec address, (b) what other layers protect against this threat, (c) why this layer needed. Convention rationale: without it, future architectural audits cannot quickly assess necessity vs redundancy — exactly the failure mode that produced the PR-289b-3 livelock and PR-290 over-correction in this session. Sprint 15d+ specs include Threat Model section by default. **v4 zip generated and uploaded** with 31 specs (26 active + 5 README). v3→v4 deltas: 3 dropped (PR-289b-3, PR-289b-4, PR-299), 3 added (PR-290a/b/c), 4 textually updated (PR-291, PR-301, PR-303, PR-304, PR-307 with text references to PR-290a/b/c per pattern attribution), threat model sections added to PR-290a/b/c/PR-291. **MCP validate_task_spec validation on PR-290a/b/c/PR-291 passes** (schema clean, 8 anti-patterns clean). **v4 upload sequence chaos:** dashboard upload attempted while daemon in CODING state, route failed to commit (waits for IDLE). Operator did manual sed for status:DONE backfill on already-merged PR-289a/PR-289b-1/PR-289b-2/PR-293..298 (8 specs reset to TODO by v3 upload earlier) + manual rm of 4 deprecated. Direct unzip with `-n` flag from chat-attached zip onto AI-Server disk was preferred path but operator chose restart-and-retry-dashboard-upload to avoid scp friction. End-state: 11 v4 specs (PR-290a/b/c, PR-291, PR-300, PR-301, PR-302a, PR-303, PR-304, PR-305, PR-307) NOT YET on disk after restart; 9 already-DONE specs preserved correctly; PR-289b-3, PR-289b-4, PR-290, PR-299 deprecated removed correctly; daemon currently CODING on PR-300 (picked from upload that landed in working tree but not yet committed by upload route). Reconciliation continuation deferred to next session. **3 new PR-FUTURE candidates derived from this session's failure modes:** PR-FUTURE-RESTART-BUTTON (~145 LoC, runtime reset endpoint + UI: aborts current operation, clears runtime Redis keys, resets workdir to origin/main, re-scans tasks; preserves content state, status frontmatter, done_index, event log), PR-FUTURE-UPLOAD-PRESERVE (~50 LoC, dashboard upload route merges existing DONE/merged/in_review frontmatter with new body on collision; only overwrites TODO/blocked/canceled/error specs — directly addresses the v4 status:TODO overwrite bug), PR-FUTURE-MCP-STATUS (~50 LoC, MCP tool `get_repo_task_status(repo_slug)` returns map of frontmatter status per spec; LLM clients call before zip preparation to validate against accidental merged-spec inclusion). All 3 sized for ~5 hours combined as Sprint 16 Batch A "Reliability layer". **OBS-CR pattern triple-confirmed** — closed without merge in 3 separate incidents: PR-275 (cross-repo intent regex on natural language, 21 iterations, 2026-05-07), PR-289b-3 (force-push regex on shell command parsing, 22 iterations, 2026-05-10), PR-299 (MCP scans negation context, 22+ iterations, 50+ threads, 2026-05-10). Pattern is now systematically deprecated: regex on natural language for safety-critical detection in a strict-review automated coder pipeline is architecturally unsuitable; defense in depth shifts to multi-layer behavioral observation (pre-push hook + stdout scan + diff content scan) rather than perfecting single-layer intent detection. **OBS-CY added** — dashboard upload route silently overwrites status:DONE frontmatter when same-named spec exists in upload zip. Root cause: route does naive file replacement on collision; no merge logic for daemon-written frontmatter fields. Fix scoped as PR-FUTURE-UPLOAD-PRESERVE above. **OBS-CZ added** — dashboard upload route waits for IDLE state before committing extracted zip but provides no operator visibility into pending uploads. When daemon is CODING, operator sees "Uploaded N files" event but files remain uncommitted in working tree; subsequent IDLE-tick commits but creates window for accidental disk drift if operator rebuilds container or restarts in between. Mitigation: operator-facing "Upload pending IDLE" badge in dashboard, possibly with Cancel option. ~30 LoC, Sprint 16. **OBS-DA added** — MCP server has no `get_repo_task_status` tool. LLM clients preparing task spec zips cannot programmatically detect "this spec id is already merged on GitHub" before upload, leading to accidental status:DONE → status:TODO regressions on every spec the LLM regenerates. Fix scoped as PR-FUTURE-MCP-STATUS above. **OBS-DB added** — daemon picker has no merged-PR sanity check before dispatching coder. After upload, picker sees status:TODO frontmatter and dispatches without verifying that the branch corresponding to the spec is already merged on GitHub. This is a regression of OBS-CT (which was closed by MICRO #344's GraphQL probe fix) re-surfaced in a new failure mode: GraphQL probe works for already-shipped PRs, but if status frontmatter is locally TODO and the branch was merged 24+ hours ago by a previous session, picker doesn't query GraphQL until WATCH cycle. Investigation deferred — possibly mitigated by Sprint 15b Phase 2 frontmatter status writer landing. **Sprint 15c v4 estimate updated** to ~28 PRs total: PR-290a/b/c + PR-291 + PR-300 + PR-301 + PR-302a/b/c + PR-303 + PR-304 + PR-305 + PR-306 + PR-307 + PR-308a/b + PR-310 + PR-311 + PR-312a/b. Operator UX chain (PR-305..PR-310) depends on Tier 2 chain (PR-301..PR-304). Standalone (PR-300, PR-311, PR-312a) can ship in parallel. Estimated 35-45 daemon-hours remaining for Sprint 15c v4 completion.).

Предыдущее: 2026-05-07 evening (Sprint 15b Phase 1 architecture decisions finalized after multi-session prior-decision recovery + operator pushback corrected several Claude-side hallucinations on operator-set BLOCKED states and BLOCKED-with-availability-fork. **Variant D extended:** file frontmatter `status` field carries 3 values — `queued` (= TODO, default absence), `merged` (= DONE, daemon writes on merge commit), `error` (= ERROR, daemon writes on final failure via ruamel.yaml + commit). Operator never edits `status` directly; field is daemon-written, operator-readable for visibility on Redis flush survival. The 6-value frontmatter parser in current code (`_FRONTMATTER_STATUS_VALUES` at queue_parser.py:35: queued/in_progress/in_review/merged/blocked/canceled) collapses to 3 — in_progress/in_review/blocked/canceled drop. derive_task_status simplifies accordingly (task_status.py:78-85). **HUNG state removed entirely** from PipelineState enum (models.py:24). All `_escalate_to_hung` callsites (watch.py:324 review timeout, fix_escalation.py NO_PUSH_DEADLOCK, fix_supervision.py:220 PR closed externally, fix.py FIX iteration cap) convert to `record_cancellation_cause + state=IDLE` continuing with next eligible task. Recover button per-repo + endpoint /repos/{name}/recover removed. Migration script for existing HUNG repos in Redis on first deploy. **ESCALATE category absorbs HUNG semantics** with `payload.subsource = "coder" | "daemon"` differentiating coder-explicit-`[ESCALATE]`-marker from daemon-detected-stuck. **Cancellation cause categories finalized at 5:** CRASH, ESCALATE (with subsource), TIMEOUT, INFRA, NO_PUSH_DEADLOCK. Removed: HUNG (merged into ESCALATE), OPERATOR_RECOVERY (Recover button gone). **Sprint 14 cancellation policy daemon-availability-fork SCRAPPED.** Original plan: ESCALATED-halt for active operator vs CANCELED-skip for off operator at the trigger moment per CRASH/ESCALATE/TIMEOUT/INFRA. Operator decision 2026-05-07: always SKIP regardless of availability. Re-purpose ManualOverrideSource: AWAY (off) → daemon continues skip without alerting; AVAILABLE (active) → daemon auto-pauses repo when ERROR rate in window exceeds operator-config threshold. is_operator_available() and 3 SignalSource implementations preserved only for UI presence chip + the new auto-pause-on-rate trigger. **Operator triage workflow finalized:** (a) Re-upload (changed file content) triggers ERROR→queued via repo_ops.py:393-398 — validator now refuses identical re-upload via SHA-256 content hash check, message "File unchanged. Use Retry button to re-attempt without changes." (b) Per-task Retry button in Tasks queue UI for ERROR badges — POSTs /repos/{name}/tasks/{pr_id}/retry, clears cancellation_cause + frontmatter status:error→queued, retry counter cap N=3 in Redis, resets on file content change. **fix_iteration_cap default raised from 15 to 25** in src/config.py:125 and config.yml:18 (config.production.example.yml already 25 — drift between default and example caused production at 15 unless operator manually copied example; included as MICRO PR before Phase 2 main refactor). Direct response to PR-275 21-iteration FIX deadlock — 25 gives more headroom before ESCALATE while still bounded. **Hidden-blocked-by-missing-dep surfaced** — _filter_dag_headers_with_available_dependencies (idle.py:157-191) no longer excludes headers with missing dep files; surfaced in Tasks queue UI as TODO with red marker on unresolved deps so operator sees the cascade silently breaking the queue. Picker logic unchanged — get_eligible_tasks still gates on "all deps DONE", so missing-dep tasks remain unpicked but visible. **Run record schema expansion (Sprint 15b Phase 2 PR #8):** 8 new fields added to metrics:run:* hash — `outcome` (merged/failed/paused/superseded), `cause` (mirrors CancellationCause when failed, NULL otherwise), `run_phase` (coding/fix/merge/recovery), `attempt_index`, `coder_session_id` (links to CLI log file path), `base_sha` + `head_sha`, `task_spec_hash`. Replaces `exit_reason` (current 9 inconsistent values mixing causes/states/operator-actions). One-time backfill script for existing 30d records ships with the migration. New Redis Set index `metrics:task_runs:{repo}:{task_id}` for retrieval performance. TTL `metrics:run:*` extended from 90d to 365d for analytics layer (Vision A territory) — 24 MB/year storage cost negligible. **Cost-per-merged-PR analytics now mechanically possible post-deploy:** filter outcome=merged, sum tokens by task_id via index, multiply by model pricing (pricing model itself remains Sprint 19+ Vision A territory but data substrate ready). Failure rate by cause / coder / complexity all queryable. **OBS-CS resolved** — Status field design closed with Variant D extended above. **OBS-CX added** — fix_iteration_cap config drift (default 15 in config.py + config.yml vs 25 in production example file). MICRO PR aligns. **Sprint 15b Phase 1 decision document COMPLETE.** Phase 2 = 10 PRs implementing the refactor + 1 MICRO (cap), ~22-28 daemon-hours estimate. Old PR-276..PR-299 specs (24 files, all linearly chained, all stuck behind PR-276's missing PR-275a/b deps) operator will renumber to PR-286..PR-309 after Phase 2 specs land at PR-276..PR-285. Detailed Phase 2 PR breakdown in new section "Sprint 15b Phase 1 finalized decisions (2026-05-07)" added below.).
...
| Sprint 15b | **Phase 1 (decision session) DONE 2026-05-07 evening:** Variant D extended (file frontmatter status: queued/merged/error, daemon-written), HUNG state removal, ESCALATE absorbs HUNG semantics with payload.subsource, 5 final cancellation cause categories, Sprint 14 daemon-availability-fork at trigger moment scrapped + re-purpose as ERROR-rate-threshold auto-pause in AVAILABLE mode, Re-upload changed-only enforcement + per-task Retry button (mutually exclusive operator intents), hidden-blocked-by-missing-dep surfaced in UI, run_record schema 8-field expansion replacing exit_reason, fix_iteration_cap default 15→25 (config drift). **Phase 2 (10 PRs + 1 MICRO implementing the refactor) DONE 2026-05-08..09** as PR-280 (#378), PR-281 (#379), PR-282 (#384), PR-283 (#380), PR-284 (#381), PR-285 (#382), PR-286 (#383), PR-287 (#385) — verified via fresh code inspection 2026-05-10: HUNG removed from PipelineState enum (models.py:12-19 has 8 states: PREFLIGHT/IDLE/CODING/WATCH/FIX/MERGE/ERROR/PAUSED), backward-compat migration kept at parse time (models.py:195-196 maps "HUNG" → PipelineState.ERROR). `_FRONTMATTER_STATUS_VALUES = {"todo", "done", "error"}` at queue_parser.py:39 (note: implementation uses `todo/done/error` lowercase rather than design-doc `queued/merged/error` — same semantics, different spellings). `write_frontmatter_status(task_path, status)` writer at queue_parser.py:84. CancellationCause categories at storage.py: CRASH, ESCALATE (with payload.subsource), TIMEOUT, INFRA, NO_PUSH_DEADLOCK — 5 final categories. **SUPERSEDED 2026-05-15..17 by PR-315..PR-320 chain — see Sprint 15c row below.** Operator post-deploy action: renumber existing tasks/PR-276..PR-299 (24 files stuck behind PR-275a/b missing deps) to tasks/PR-286..PR-309 — DONE during Phase 2 ship. | **Phase 1 DONE 2026-05-07. Phase 2 DONE 2026-05-08..09.** | Phase 2 shipped — actual estimate ~22-28 daemon-hours validated. |
| Sprint 15c | **v4 reorganization 2026-05-10:** layered protection model with 7 layers documented. v4 spec set: PR-290a/b/c (Tier 1 diff scan split: infrastructure + workflow patterns + governance/supply-chain), PR-291 (AGENTS.md forbidden_actions section with 7 prohibitions), PR-300 (main commit attestation audit), PR-301 (Tier 2 large-diff threshold), PR-302a/b/c (Tier 2 secrets Groups A/B/C), PR-303 (Tier 2 mass-deletion), PR-304 (Tier 2 governance file extensions: CODEOWNERS, dependabot, labels, settings), PR-305 (operator override backend), PR-306 (operator override UI), PR-307 (notification webhook), PR-308a/b (cascade ESCALATE detector + UX), PR-309 SHIPPED, PR-310 (GUARDRAIL distinct dashboard filtering), PR-311 (git bundle backups), PR-312a/b (bubblewrap sandbox helper + integration), PR-313 SHIPPED. Threat Model section convention introduced. **PR-289b-3 + PR-289b-4 + PR-299 + original PR-290 monolith dropped** as OBS-CR class deprecated or over-correction. Polish bucket fully shipped (PR-292..PR-298). **PR-315..PR-320 chain shipped on top (verified 2026-05-17):** further category collapse from 5-value vocabulary to single `category="ERROR"` + `payload.subsource` discriminator. PR-315 (collapse CancellationCause vocabulary), PR-316 (WATCH review_timeout callsite migration to `_transition_to_error`), PR-317 (remove `_escalate_and_skip`, convert 7 legacy callsites), PR-318 (recovery handler dispatches on `payload.subsource`), PR-319 (cancellation_card.html UI dispatch on subsource with legacy_category fallback), PR-320 (docs update: TASK_SCHEMA + operations + AGENTS reflect single-ERROR model). Effective new vocabulary: single `category="ERROR"` with `payload.subsource` ∈ 8 canonical (`crash`, `coder_escalate`, `guardrail`, `review_timeout`, `fix_idle_timeout`, `fix_iteration_cap`, `no_push_deadlock`, `infra_failure`) plus 3 out-of-vocabulary write sites (`daemon` migration-only, `watch_retrigger_cap` PR-310-era detector, `operator_reject` PR-305c operator action). Dashboard filter groups these 11 into 4 buckets per `_SUBSOURCE_FILTER_GROUPS` at `src/web/routes/dashboard.py:82-98`. **Sprint 15c v4 batch COMPLETE 2026-05-17 (all 28 PRs status:DONE in tasks/).** Effective remaining queue: only PR-FUTURE-RESTART-BUTTON, PR-FUTURE-UPLOAD-PRESERVE, PR-FUTURE-MCP-STATUS remain queued for Sprint 16 Batch A "Reliability layer" per 2026-05-10 entry. | **Sprint 15c #1 batch DONE 2026-05-09..10** (PR-289a, PR-289b-1, PR-289b-2, PR-292..298, PR-288, PR-309, PR-313 = 12 PRs). **Sprint 15c v4 batch DONE 2026-05-10..15** (PR-290a/b/c, PR-291, PR-300..PR-304, PR-305a/b/c, PR-306..PR-308b, PR-310, PR-311a/b, PR-312a/b = ~22 PRs). **Sprint 15c PR-315..PR-320 follow-up chain DONE 2026-05-15..17** (6 PRs single-ERROR collapse). | All shipped. ~38 daemon-hours total Sprint 15c. |
| Sprint 15c ext | Webhook + operator_reject visibility additions (2026-05-20 chat-session, specs MCP-validated): PR-355 (webhook delivery audit log to `/data/audit/webhooks/{event_type}/YYYY-MM-DD.jsonl` with URL hashing to avoid leaking webhook secrets, fcntl.flock concurrent-write safety), PR-356 (webhook test button in Settings with inline delivery status, depends on PR-344b Notifications section + PR-355 audit log), PR-357 (operator-reject forensic history page at `/repo/{name}/rejects` listing historic operator rejections with rule and excerpt, reuses cancellation_index for discovery). All Low risk, additive. | Specs ready, awaiting upload | 3 PRs, ~6 daemon-hours estimated |
...
| Sprint 15d ext | Reliability hardening + production-incident closures (2026-05-20 chat-session, specs MCP-validated): PR-351 (backup branch fallback when feature-branch push rejected by branch protection, `crash-backup/{task_id}/{ts}` naming, Redis metadata for dashboard surface), PR-352 (pre-merge-sync sub-state visibility with `merge_phase` field on RepoState rendered as subtitle under MERGE badge), PR-353 (bubblewrap sandbox 3-state badge: disabled/active/unavailable; runtime smoke test at boot + config reload via PR-342 inotify; depends on PR-342), PR-354 (main commit audit findings panel at `/repo/{name}/audit` for direct-to-main commit forensic review), **PR-358 (single @codex review repost on review_timeout before terminal ERROR — closes OBS-DO, restores HUNG-era repost behavior lost in PR-276/277, uses existing HungMixin `_post_codex_review(bypass_same_head_dedup=True, bypass_author_dedup=True)`, new RepoState field `review_timeout_repost_attempted: bool=False` reset on FIX/MERGE entry + new-PR transition via __setattr__ hook, total window 40min before terminal ERROR when Codex stays silent, Priority 2)**, **PR-359 (snapshot sync in `_mark_status_write_failed_task` so Retry button appears without docker restart — closes OBS-DP, mirrors `recovery.py:_apply_recovery_decisions` override into live path via `state.current_queue = list(snapshot)` reassign + `await self.publish_state()`, Priority 2)**. PR-358 + PR-359 are production-incident closures from 2026-05-17..20 PR-330b PR #465 hang. | Specs ready, PR-358 + PR-359 P2 ship-first | 6 PRs, ~13 daemon-hours estimated |
| Sprint 15e | Subsource registry — typed metadata module for the 11 cancellation subsources (2026-05-17 chat-session, specs MCP-validated): PR-322 (Pydantic SubsourceMetadata model + registry data structure + lookup() helper + 11 entries with user_label, severity, recovery_hint, group_bucket per subsource — formalizes the planned-debt entry from `src/daemon/notifications.py:13-15`), PR-323 (refactor `_SUBSOURCE_FILTER_GROUPS` in dashboard.py to consume registry instead of hardcoded dict), PR-324 (cancellation card badge enrichment: render `user_label` instead of raw subsource string), PR-325 (completeness validation test: assert every SUBSOURCE_VOCABULARY entry + 3 out-of-vocab entries have registry entries). All Low risk, additive. Closes long-standing taxonomy debt. | Specs ready, awaiting upload | 4 PRs, ~7 daemon-hours estimated |
| Sprint 15f | WorkInhibitor unification — typed primitive for the 8 throttle mechanisms (2026-05-17 chat-session, specs MCP-validated, **HIGH risk dispatcher hot path**): PR-326 (WorkInhibitor Pydantic model + 8-value InhibitorType enum: USER_PAUSE, USER_STOP, RATE_LIMIT, SPEND_CEILING, GITHUB_BUDGET_PAUSE, GITHUB_BUDGET_SLOWDOWN, CASCADE_PANIC, ERROR_RATE_AUTO_PAUSE), PR-327 (derive_active_inhibitors helper walking Redis state into typed list, read-only no behavior change), PR-328 (RepoState.active_inhibitors field + populate at publish_state), PR-329 (is_work_inhibited dispatcher helper behind `use_unified_inhibitor_check` feature flag default False — **SHIPPED ON PRODUCTION 2026-05-20 commit de32b48, default False so handlers still on legacy path**), PR-330a/b/c/d (handler migration cutover: handle_idle, handle_paused, handle_watch wrapped with flag-gated conditional + final flip default to True after canary verification), PR-331 (per-inhibitor stacked badges with individual Clear actions on PAUSED cards replacing single Resume button). **PR-329 production-shipped via daemon autonomous pipeline 2026-05-20 (commit de32b48 included in 65-commit deploy 51b156d -> 327a710). PR-330a/b/c/d cutover NOT YET on main — PR-330b PR #465 hit review_timeout hang during canary attempt, closed by operator. Re-dispatch pending PR-358 + PR-359 ship to ensure WATCH review_timeout no longer parks intermediate.** | PR-329 DONE, PR-326..PR-328 + PR-330a/b/c/d + PR-331 spec ready | 9 PRs, ~18 daemon-hours estimated (high risk requires staged canary) |
| Sprint 16 | Config architecture three-layer split (config.yml shipped immutable / config/providers.yml / data/user_state.yml gitignored / Redis transient; OBS-BZ + OBS-DR resolution; dynamic list_models per provider plugin; auto-detect bootstrap; one-time migration; UI add-provider/add-coder wizard). **Per 2026-05-20 chat-session, Sprint 16 split into 3 batches A/B/C, each shipping independently before the full architectural split:** | Queued | 12-16 PRs total, ~26-32 daemon-hours |
| Sprint 16 Batch A | Reliability layer + diagnostic surface (2026-05-20 chat-session, specs MCP-validated): PR-332 (GET /api/diagnostic/{repo}/{task_id} returning structured JSON of all per-task Redis state — closes 8-redis-cli-commands triage pain), PR-333 (HTMX diagnostic_panel.html frontend with subsource_metadata + retry_count/cap + fingerprint_matches + PR linkage rendering), PR-334 (POST /api/reset-task/{repo}/{task_id} atomic destruction endpoint with WATCH/MULTI Redis transaction + `gh pr close` orphan PR + frontmatter -> TODO rewrite — closes PR-FUTURE-RESTART-BUTTON gap-filler per 2026-05-10 entry), PR-335 (reset confirmation modal with repo-name typed confirmation gate), PR-336 (reset action audit log to `/data/audit/operator-actions/YYYY-MM-DD.jsonl`), PR-337 (upload route preserves DONE/merged frontmatter on collision — closes OBS-CY per 2026-05-10), PR-338 (MCP `get_repo_task_status(repo_slug)` tool returning frontmatter status map — closes OBS-DA per 2026-05-10). | Specs ready, awaiting upload | 7 PRs, ~16 daemon-hours estimated |
| Sprint 16 Batch B | Global pause/stop coordination (2026-05-20 chat-session, specs MCP-validated): PR-339 (4 daemon-wide endpoints POST /daemon/pause /resume /stop + GET /daemon/drain-progress composing existing per-repo primitives), PR-340 (header global Pause/Stop/Resume buttons with confirmation modals for destructive actions, Resume reversible without modal), PR-341 (per-card drain-progress indicator on PAUSED state with `merge_phase`-like elapsed/estimated-remaining timing per CODING/FIX phase). All Low risk additive. | Specs ready, awaiting upload | 3 PRs, ~6 daemon-hours estimated |
| Sprint 16 Batch C | Config UX additions (2026-05-20 chat-session, specs MCP-validated): PR-342 (inotify config.yml hot reload via watchfiles library, reduces edit-to-active latency from 25s to under 2s, fallback to existing 5-cycle poll if unavailable — also enables PR-353 sandbox state refresh on config reload), PR-343 (event log mirror to disk JSONL at `/data/events/{repo}/YYYY-MM-DD.jsonl` surviving Redis flush, fcntl.flock concurrent-write safety), PR-344a (Settings page spend_ceiling fields with ruamel.yaml round-trip preserving comments), PR-344b (Settings page webhook fields with URL validation + min_tier enum), PR-344c (Settings page backup + sandbox + audit fields with bubblewrap 3-state badge integration via PR-353), PR-345 (cancellation TTL refresh on read for 90-day forensic window, opt-in `refresh_ttl=True` from explicit diagnostic_panel access, `refresh_ttl=False` from dashboard 5s poll to avoid pinning records). All Low risk additive. | Specs ready, awaiting upload | 6 PRs, ~12 daemon-hours estimated |
...
- OBS-CX added 2026-05-07 evening — `daemon.fix_iteration_cap` config drift between default and example. Default 15 in src/config.py:125 + config.yml:18. Production example file config.production.example.yml:24 has 25. If operator's actual production config inherits default rather than copying example, runs at 15. PR-275 21-iteration FIX deadlock validated need for higher cap. Sprint 15b MICRO PR aligns both default sources to 25. **CLOSED 2026-05-08 by Sprint 15b Phase 2 MICRO** — verified via fresh code inspection 2026-05-10: `src/config.py:134` reads `fix_iteration_cap: int = Field(default=25, ge=1)`, `config.yml` reads `fix_iteration_cap: 25`. Both default sources aligned to 25. Status: CLOSED.

**Renumbering record (2026-05-10):**
- **Sprint 15b Phase 2 verified shipped via fresh code inspection 2026-05-10** — 8 PRs (PR-280..PR-287) on disk + merged on GitHub. HUNG state removed from PipelineState enum (now 8 states: PREFLIGHT/IDLE/CODING/WATCH/FIX/MERGE/ERROR/PAUSED at models.py:12-19); backward-compat parse migration kept (models.py:195-196 maps legacy Redis "HUNG" → ERROR); `_FRONTMATTER_STATUS_VALUES = {"todo", "done", "error"}` at queue_parser.py:39 (implementation uses lowercase `todo/done/error` rather than design-doc `queued/merged/error` — same semantics, different spellings); `write_frontmatter_status(task_path, status)` daemon-side writer at queue_parser.py:84; 5 final cancellation causes in src/cancellation/storage.py: CRASH, ESCALATE, TIMEOUT, INFRA, NO_PUSH_DEADLOCK. Spec table notation `queued/merged/error` should read `todo/done/error` per current implementation — semantic equivalence preserved but spellings differ from Phase 1 design doc.
- **OBS-CU CLOSED 2026-05-08** by Sprint 15b Phase 2 MICRO — verified via fresh code inspection 2026-05-10: call site `self._scan_task_specs_for_agents_md_drift()` no longer present in `src/daemon/handlers/idle.py`. Method definition retained at idle.py:85 (consumed by MCP `validate_task_spec` at upload time) but no longer invoked on IDLE-tick.
- **OBS-CX CLOSED 2026-05-08** by Sprint 15b Phase 2 MICRO — verified via fresh code inspection 2026-05-10: `src/config.py:134` reads `fix_iteration_cap: int = Field(default=25, ge=1)`, `config.yml` reads `fix_iteration_cap: 25`. Both default sources aligned.
- **PR-289b-3 (force_push_main Tier 1 stdout pattern)** — entered 22-iteration FIX deadlock on PR #397, 10+ Codex review concerns on refspec semantics. Closed PR #397 without merge, dropped from queue. Architectural reasoning: pre-push hook (PR-272 shipped) already validates branch identity at git push time including refspec deletion forms and HEAD:branch destinations; stdout regex catalogue is wrong abstraction for parsing git push semantics. OBS-CR pattern confirmed.
- **PR-289b-4 (direct_commit_main sequence detector)** — dropped without implementation as architecturally symmetric to PR-289b-3, redundant with pre-push hook's expected-branch contract.
- **PR-299 (MCP scans negation handling extension)** — entered 22+ iteration FIX deadlock on PR #404, 50+ unresolved Codex review threads on negation/dirty-context trade-off boundaries. Closed PR #404 without merge, dropped from queue. OBS-CR class — same architectural failure mode as PR-275 and PR-289b-3.
- **PR-290 (post-push diff content scan, original v3 monolith) split into PR-290a/b/c per 150-LoC cap.** PR-290 was initially deleted in over-correction conflating its diff-content scope with the force-push concerns of PR-289b-3; realized via dashboard "Blocked by: PR-290" cascade across 5+ specs (PR-291, PR-301, PR-303, PR-304, PR-307) plus body-text references in PR-303/PR-304 explicitly naming PR-290's workflow_destruction, permissions_escalation, branch_protection_modification patterns as architectural primitives. Restored as 3 narrow-scoped PRs: PR-290a (scan_pr_diff infrastructure + empty catalogue + WATCH integration, ~160 LoC), PR-290b (workflow YAML patterns: permissions_escalation + workflow_destruction, ~142 LoC), PR-290c (governance + supply chain: branch_protection_modification + dangerous_action_external_install with _PINNED_ACTION_REF_RE allowlist for semver tags and 40-char commit SHAs, ~158 LoC).
- **OBS-CR pattern triple-confirmed** as systemic architectural class — closed without merge in PR-275 (2026-05-07), PR-289b-3 (2026-05-10), PR-299 (2026-05-10). Regex on natural language for safety-critical detection is incompatible with strict-review automated coder pipeline. Defense-in-depth strategy: multi-layer behavioral observation (pre-push hook + stdout scan + diff content scan) instead of single-layer intent detection.
- **7-layer protection model documented in roadmap.** Layer 1 = MCP validate_task_spec, Layer 2 = AGENTS.md prompts (PR-291), Layer 3 = pre-push hook, Layer 4 = Tier 1 stdout scan, Layer 5 = Tier 1 diff content scan (PR-290a/b/c), Layer 6 = Tier 2 diff content scan (PR-301..PR-304), Layer 7 = Operator UX (PR-305..PR-310).
- **Threat Model section convention introduced** for guardrail specs starting v4. Each spec answers (a) what threat does this spec address, (b) what other layers protect against this threat, (c) why this layer needed. Convention prevents the architectural-audit gap that produced the PR-289b-3 livelock + PR-290 over-correction.
- **3 new PR-FUTURE candidates derived from v4 upload incident:** PR-FUTURE-RESTART-BUTTON (~145 LoC, runtime reset endpoint + UI), PR-FUTURE-UPLOAD-PRESERVE (~50 LoC, dashboard upload preserves DONE/merged/in_review frontmatter), PR-FUTURE-MCP-STATUS (~50 LoC, MCP tool returns spec status map). Combined ~5 hours as Sprint 16 Batch A "Reliability layer".
- **OBS-CY added** — dashboard upload route silently overwrites status:DONE frontmatter on collision. Fix scoped as PR-FUTURE-UPLOAD-PRESERVE.
- **OBS-CZ added** — dashboard upload waits for IDLE state before committing extracted zip but provides no operator visibility into pending uploads. Sprint 16 cosmetic fix.
- **OBS-DA added** — MCP server has no `get_repo_task_status` tool, allowing LLM clients to accidentally regress merged specs to status:TODO in regenerated zips. Fix scoped as PR-FUTURE-MCP-STATUS.
- **OBS-DB added** — daemon picker has no merged-PR sanity check before dispatching coder when frontmatter says status:TODO but corresponding branch is already merged on GitHub. Regression of OBS-CT class in new surface. Investigation deferred pending Sprint 15b Phase 2 frontmatter status writer landing.

**Renumbering record (2026-05-17):**
- **PR-315..PR-320 chain VERIFIED SHIPPED via code inspection** (status:DONE in tasks/, never enumerated in Sprint 15c v4 backlog). Further category collapse from PR-315b Phase 2's 5-value vocabulary to single `category="ERROR"` with `payload.subsource` discriminator. Verified: `src/cancellation/storage.py:42-50` CancellationCause.category is bare `str` accepting any value; 14 callsites in src/ pass `category="ERROR"`; 0 non-migration callsites use the old 5 values (CRASH/ESCALATE/TIMEOUT/INFRA/NO_PUSH_DEADLOCK). `_escalate_and_skip` function fully removed per PR-317 — 4 remaining mentions in src/ are docstrings explaining the removal. `_transition_to_error` is unified path with 57 callsites. `src/web/templates/components/cancellation_card.html` has 8 subsource branches matching the canonical PR-315 vocabulary, plus `legacy_category` fallback path for pre-migration records and a generic "Error: <subsource>" branch for out-of-vocabulary subsources.
- **Sprint 15b Phase 2 sprint table row amended** with forward pointer to PR-315 chain ("SUPERSEDED 2026-05-15..17 by PR-315..PR-320 chain — see Sprint 15c row below"). Phase 2's "5 final cancellation cause categories" description retained for historical accuracy at the Phase 2 ship date 2026-05-08..09 but now flagged as superseded.
- **Sprint 15c sprint table row amended** with PR-315..PR-320 enumeration, vocabulary detail (8 canonical + 3 out-of-vocab subsource write sites), and updated completion status: Sprint 15c v4 batch COMPLETE 2026-05-17 with 28 PRs status:DONE plus PR-315..PR-320 follow-up chain. Effective remaining queue: PR-FUTURE-RESTART-BUTTON, PR-FUTURE-UPLOAD-PRESERVE, PR-FUTURE-MCP-STATUS only, moved to Sprint 16 Batch A "Reliability layer".
- **OBS-DK status update (still OPEN as of 2026-05-17):** code inspection of `src/daemon/fix_supervision.py:33-97` confirms the regression NOT YET FIXED. Function `monitor_fix_idle` still has the broken baseline init at lines 60-64 (`backdate = min(head_age, idle_limit - poll_interval)`, `last_known_push = now - max(0.0, backdate)`). Production FIX iterations remain globally broken. Fix scope from 2026-05-15 entry (5 LoC change, replace baseline init with `last_known_push = now - head_age if head_age < poll_interval else now`) must ship before any new FIX-cycle work resumes. **Severity: CRITICAL, blocks all FIX-cycle ship.**
- **CoderPlugin Protocol generalization is ~40% done in code (Sprint 19+ Vision A prep partial):** `src/coder_registry.py:14` declares `CoderPlugin` Protocol with `rate_limit_patterns() -> list[re.Pattern[str]]` method. `src/coders/claude.py:143` and `src/coders/codex.py:193` both implement it returning provider-specific regex tuples. BUT consumer `_detect_rate_limit` in `src/daemon/rate_limit.py` does NOT consume `plugin.rate_limit_patterns()` — patterns still hardcoded inline with if/elif on `coder_name` strings. Other plugin barriers verified present: `src/daemon/selector.py:18` `_RUNTIME_SUPPORTED_CODERS = {coder.value for coder in CoderType}` (static enum gating), `selector.py:112` `rank_auxiliary_coders` hardcodes `("claude", "codex")` tuple. Sprint 19+ "Plugin Protocol generalization" work is finishing what's started, not greenfield.
- **Retry endpoint covers happy-path Reset** at `src/web/routes/repo_control.py:1028` (`POST /repos/{name}/tasks/{pr_id}/retry`): frontmatter→TODO, commit+push, DELETE cancellation:{repo}:{task_id}, clear status_write_failed marker, publish wake, increment retry counter capped at retry_button_cap=3. Does NOT (yet) handle: cap-reached escape hatch (returns 409), retry_fingerprint clear, ZREM from cancellation_index, current_run_started_at delete, orphan PR close. PR-FUTURE-RESTART-BUTTON (~145 LoC, Sprint 16 Batch A) is the gap-filler for stuck-after-retry-cap cases.
- **SUBSOURCE_VOCABULARY canonical/actual gap quantified:** `src/cancellation/__init__.py:47` SUBSOURCE_VOCABULARY frozenset has 8 entries (canonical PR-315 set). Three additional subsource strings circulate in code: `daemon` (migration-only, `src/daemon/migrations/hung_to_idle.py`), `watch_retrigger_cap` (newer detector at `src/daemon/handlers/watch.py:707`, PR-310-era), `operator_reject` (operator action at `src/web/routes/repo_control.py:1707`, PR-305c). Dashboard filter `_SUBSOURCE_FILTER_GROUPS` at `src/web/routes/dashboard.py:82-98` explicitly groups all 11 into 4 buckets (guardrail/coder/daemon/operator_reject) with comment at dashboard.py:106 acknowledging these "sit outside the canonical PR-315 SUBSOURCE_VOCABULARY". PR-307a comment at `src/daemon/notifications.py:13-15` documents this as planned debt: "spec names overlap partly with src/daemon/guardrails.py emitted category strings; a follow-up PR can replace string parsing with structured cause-payload fields and reconcile the taxonomy." Candidate Sprint slot: formalize the 11-subsource vocabulary as data structure with metadata (user_label, severity, recovery_hint, group_bucket). Sprint 19+ candidate or earlier follow-up depending on priority.
- **MCP suggest_next_pr_number returns 321** at session time per `mcp__pipeline-orchestrator-mcp__suggest_next_pr_number(repo="AlexBomber12__pipeline-orchestrator")` call. PR-321.md absent from tasks/ (shipped 2026-05-15 commit 32d1061, file removed on merge per Variant D extended convention). PR-314.md gap (likely shipped earlier without retention or operator-removed). Highest existing in tasks/ is PR-320.md. Next-free integer per MCP scan is 321. Recommendation for new spec generation: use PR-322 to avoid commit-log narrative collision with the 2026-05-15 PR-321 ship, accepting one wasted number for clarity.
- **OBS-DN added (LOW, methodology):** roadmap sprint table row summaries can lag the text body which can lag the code. The 2026-05-17 chat-session that produced v4/v5 implementation plans referenced the line 47 Sprint 15b Phase 2 summary ("5 final cancellation causes") and proposed a "drop legacy_category fallback" intervention that PR-315 had already shipped. Roadmap text body at lines 121, 130 referenced PR-315/PR-318 in passing, but the sprint table itself was not amended. Triple-source-verification (table → text body → code) required when planning new architectural work that depends on existing primitives. Fix candidate: when a chain of refactor PRs (like PR-315..PR-320) closes a multi-PR theme, update the relevant earlier sprint table row to point forward ("see PR-XXX update entry") at the same time the new entry is added. Cosmetic, ~10min editing per shipped chain.
- **OBS-DO added (HIGH, regression):** WATCH review_timeout silently parks task with no automatic recovery attempt. Feature regression from PR-276/PR-277 (Phase 2 HUNG state removal) which lost the "post @codex review once before escalating" behavior previously hosted in `PipelineState.HUNG` handler. Production incident 2026-05-17..20: PR-330b PR #465 hung 21 minutes with review=PENDING, ci=SUCCESS, Codex never posted review, daemon transitioned to terminal ERROR with subsource=review_timeout requiring operator Retry. Closed by PR-358 (Sprint-15d-ext, Priority 2, MCP-validated, specs ready): single force-post `@codex review` via existing HungMixin `_post_codex_review(bypass_same_head_dedup=True, bypass_author_dedup=True)` on first 20m timeout, reset window via `current_pr.last_activity=now`, new `review_timeout_repost_attempted: bool=False` field on RepoState with `__setattr__` hook reset on new-PR transition + `current_task=None`. Second 20m timeout transitions to terminal ERROR with `repost_attempted: true` in cancellation payload (forensic). Total window before terminal ERROR when Codex stays silent: 40 minutes. Reset points: `handle_fix` entry + `handle_merge` entry. Recovery handler unchanged (DOING + open PR resume to WATCH already correct).
- **OBS-DP added (HIGH, regression):** in-memory ERROR fallback marker (`_status_write_failed_task_pr_ids`) invisible to operator until docker restart. `_mark_status_write_failed_task` at `src/daemon/runner.py:1755-1765` registers the marker in Redis but does NOT sync `state.current_queue` snapshot. Override IS applied correctly on 2 paths (recovery boot at `recovery.py:384-404`, IDLE DAG rebuild at `idle.py:520-526`) but live write path missing the sync. Dashboard reads from `state.current_queue` so task renders under TODO group with "Blocked by" badge, Retry button (rendered only for `status_key == 'error'` per `tasks_panel.html:32`) invisible. After `docker compose restart daemon`, boot recovery applies the override, Retry button appears. Production incident 2026-05-17..20: PR-330b after review_timeout terminal ERROR, status commit to main rejected by branch protection (no signed commit / no PR review), in-memory ERROR fallback registered but operator saw no Retry button until restart. Closed by PR-359 (Sprint-15d-ext, Priority 2, MCP-validated, specs ready): inline-update `state.current_queue` mirroring `recovery.py:_apply_recovery_decisions` override logic, reassign `state.current_queue = list(snapshot)` to retrigger RepoState `__setattr__` re-stamp of `current_queue_snapshot_at`, then `await self.publish_state()`. Retry button appears on next dashboard 5s poll. Defensive DONE discard mirrors `recovery.py:394-396` for the edge case where status_write_failed marker collides with a DONE task.
- **OBS-DQ added (LOW, methodology):** MCP `validate_task_spec` schema-validates but does NOT catch user-preference violations (em-dashes U+2014, en-dashes U+2013, Unicode arrows U+2192/U+2190, mathematical symbols). During 2026-05-20 chat-session, 33 specs generated with em-dashes initially (typewriter habit from prose-style markdown), required post-generation Unicode-cleanup pass before MCP validation could be confidently claimed "passed". Methodology fix: pre-MCP grep `python3 -c "for c in content: if ord(c) > 127: print"` ASCII-purity check, OR sed-pass converting common Unicode dashes/arrows to ASCII equivalents before submitting to validate_task_spec. Stub-validation of MCP gives false confidence — passing a 10-line stub through validate_task_spec proves only that the header parses; AGENTS.md anti-pattern check on the full body requires the full body to be sent. Cosmetic, but accumulates across large spec batches.
- **OBS-DR added (MEDIUM, operations):** `config.yml` on production AI-Server has `skip-worktree` flag (per OBS-BZ from 2026-05-04 — operator runtime overrides for coder=claude pin + large_diff thresholds + per-repo feature flags). Upstream commit on main (PR-329 ship `de32b48`) removed per-repo `feature_flags: use_unified_inhibitor_check: false` blocks from canonical config.yml (became redundant after FeatureFlags default False in `src/config.py`). Operator pull blocked by `git pull` claiming "local changes would be overwritten" while `git diff config.yml` reported empty. Root cause: skip-worktree masks the actual content divergence (operator had explicit `feature_flags:` blocks in production config.yml, upstream removed them); without skip-worktree the file would appear as modified and stash+pop would unwedge cleanly. Reconciliation sequence: `git update-index --no-skip-worktree config.yml && git checkout origin/main -- config.yml && git pull && git update-index --skip-worktree config.yml`. Sprint 16 three-layer config split (config.yml shipped immutable / config/providers.yml / data/user_state.yml gitignored / Redis transient) remains the architectural fix. Until then, document the unwedge procedure prominently in `docs/operations.md`. Backlog MICRO ~30 LoC: dedicated docs section "Operator: how to reconcile config.yml skip-worktree on upstream pull".

---

## Текущий статус

- **Production deployed and stable** since 2026-04-29. Sprint 12 Foundation (36 PRs, PR-208..PR-236) shipped 2026-05-04. Sprint 13/13.5/14 (23 PRs, PR-238..PR-260) shipped and verified 2026-05-04. Sprint 15a #6 + 15a.5 (16 PRs, PR-258a..PR-273) shipped 2026-05-06 on commit cb02e07. Sprint 15a #7 (PR-274) shipped 2026-05-07 as PR #365. 2026-05-07 hotfix bundle shipped (#343 #344 #370). **Sprint 15b Phase 2 shipped 2026-05-08..09** (8 PRs: PR-280..PR-287) — frontmatter status writer (todo/done/error), HUNG state removed from PipelineState enum (8 states remaining), 5 final cancellation cause categories (subsequently collapsed further by PR-315 chain — see below), ESCALATE absorbs HUNG semantics with payload.subsource, OBS-CU AGENTS-SCAN silence MICRO shipped (call site removed), OBS-CX fix_iteration_cap config drift closed (default 25 aligned). Sprint 15c #1 batch (12 PRs: PR-288, PR-289a, PR-289b-1, PR-289b-2, PR-292..298, PR-309, PR-313) shipped 2026-05-09..10. **Sprint 15c v4 batch fully shipped 2026-05-10..15** (~22 PRs: PR-290a/b/c, PR-291, PR-300..PR-304, PR-305a/b/c, PR-306..PR-308b, PR-310, PR-311a/b, PR-312a/b — all status:DONE per 2026-05-17 code inspection). **Sprint 15c PR-315..PR-320 follow-up chain shipped 2026-05-15..17** (6 PRs collapsing CancellationCause to single ERROR + payload.subsource discriminator; `_escalate_and_skip` removed; `_transition_to_error` unified path with 57 callsites; cancellation_card.html dispatches on 8 canonical subsource branches plus legacy_category fallback). **Production deploy 2026-05-20 (commit 51b156d → 327a710, 65 commits, fast-forward):** brings Sprint 15c v4 batch + PR-315..PR-320 single-ERROR chain + PR-329 unified inhibitor helper (`use_unified_inhibitor_check` feature flag default False per spec — handlers still on legacy path) to production. PR-353 (bubblewrap 3-state badge) branch `pr-353-bubblewrap-3state-badge` present on origin. PR-330b branch `pr-330b-handle-paused-migration` (PR #465) closed by operator during 2026-05-17..20 incident — see below. **Critical pending fixes:** PR-358 + PR-359 (Sprint-15d-ext, Priority 2, MCP-validated, specs ready) closing OBS-DO + OBS-DP production-incident regressions. Upload Sprint-15d-ext.zip to daemon pipeline ASAP — both P2, no deps between them, ship-eligible same-day. Total ~411 PRs merged in production (51b156d → 327a710 added 65, prior baseline ~346 + 27 hotfixes/Sprint 15c v4 follow-up).
- **Daemon state at 2026-05-20:** production daemon running commit 327a710 after `docker compose up -d --build`. PR-329 unified inhibitor helper on production, default False — handlers still on legacy path until canary verification of PR-330a/b/c/d cutover. **PR-358 + PR-359 ship-pending (P2 production-incident closures).** OBS-DK fix status uncertain (was pending 2026-05-17, may have shipped in 65-commit deploy; verify via direct grep on `src/daemon/fix_supervision.py:51-61` post-deploy). After PR-358 + PR-359 deploy: re-dispatch PR-330b via Sprint-15f.zip re-upload to clear cancelled state; continue Sprint 15f cutover only after canary verification of PR-358 (no auto-Retry incidents in 24h on test-app or first repo with PR-358 deployed). **Critical pending work for next session:** (1) Verify OBS-DK fix status in production deployed code (grep `monitor_fix_idle` baseline init). (2) Upload Sprint-15d-ext.zip containing PR-358 + PR-359 ASAP. (3) After PR-358 + PR-359 deploy, re-dispatch PR-330b. (4) Sprint 15e (PR-322..PR-325 subsource registry) ship anytime — fully standalone, no deps, no behavior change. (5) Sprint 16 Batch A/B/C and Sprint 15c/15d ext remaining PRs in any parallel-eligible order, max parallelism 2.
- **Sprint 15c v4 reorganization 2026-05-10 (closed):** 7-layer protection model finalized, PR-290 split into PR-290a/b/c per 150-LoC cap, PR-289b-3 + PR-289b-4 + PR-299 dropped as OBS-CR class, Threat Model section convention introduced. v4 zip 31 specs generated and validated via MCP. All v4 batch + PR-315..PR-320 follow-up shipped by 2026-05-17.
...
- **OBS-CR** (regex-based natural-language intent detection inherent deadlock pattern): **OPEN, ARCHITECTURAL, observed 2026-05-07 across two consecutive deadlocks PR #368 (PR-275) and PR #371 (PR-275a); triple-confirmed 2026-05-10 across PR #397 (PR-289b-3) and PR #404 (PR-299)**. PR-275 attempted to detect cross-repo intent in task spec body via regex patterns covering phrasings like "ships in <repo>", "targets <repo>", "lives in <repo>", "deploys to <repo>", plus literal `gh repo create` commands. Codex review flagged 10 substantive P1/P2 issues — all variations of the same fundamental problem: regex pattern matching cannot reliably distinguish between executable destructive intent and innocent prose. Each fix to one false positive opened a false negative elsewhere. PR-275 went 21 review iterations, never converged. Closed PR #368, split into PR-275a/b/c/d with deliberately narrow scope (literal command match outside fenced code blocks + inline backticks). PR-275a entered same deadlock loop within 6 review iterations. Closed PR #371. **2026-05-10 confirmations:** PR-289b-3 (force_push_main stdout regex on git push semantics) entered 22-iteration FIX deadlock with 10+ Codex review concerns on `--repo` refspec semantics, `+:` matching-refs force, `HEAD:main` destination, dry-run negative-lookahead — all variations of the same shell-command-parsing-via-regex impossibility. Closed PR #397. PR-299 (MCP scans negation handling for merge_dirty_alt) entered 22+ iteration FIX deadlock with 50+ unresolved threads on negation/dirty-context trade-off boundaries — same architectural class. Closed PR #404. **Architectural conclusion (triple-confirmed):** regex-based natural-language intent detection in task specs OR shell command parsing OR negation context handling is **systemically unsuitable** for this design pattern (automated coder + automated review + strict review criteria). Codex review is independent LLM agent that finds legitimate technical concerns at every regex trade-off position; coder cannot pass review for any single design choice in this domain. **Fix architecture:** drop spec-validation regex layer for cross-repo intent entirely (done in Sprint 15b). Drop stdout regex for force-push and direct-commit (done 2026-05-10 — PR-289b-3 and PR-289b-4 removed from queue). Drop MCP negation context handling enhancement (done 2026-05-10 — PR-299 removed). Defense in depth shifts to behavioral observation layers: pre-push hook (PR-272 — catches branch identity issues at git push time), Tier 1 stdout scan (PR-289a/b — catches CLI invocations with deterministic shell command markers like `gh repo create`), Tier 1 diff content scan (PR-290a/b/c — catches workflow YAML and governance file modifications at PR diff level). All observe coder behavior at distinct enforcement points rather than parsing intent from prose. **Lessons captured:** (1) regex on natural language for safety-critical detection is incompatible with strict-review automated coder pipeline; (2) shell command parsing via regex hits the same impossibility wall as natural language parsing (refspec semantics, flag aliases, global git options, dry-run negation all admit countless variations); (3) intent detection in spec text is an LLM-classification problem, not a regex problem; (4) defense-in-depth with multiple behavioral observation layers (pre-push hook + stdout scan + diff content scan) more robust than perfecting any single regex layer; (5) when a single PR enters 20+ FIX iterations without convergence, escalate the spec to architectural review rather than continuing — wasted coder-hours and Codex review tokens become significant.
...
- **OBS-CU** (AGENTS-SCAN noise in event log — detection without response): **CLOSED 2026-05-08 by Sprint 15b Phase 2 (PR-280..PR-287 batch)**. Verified via fresh code inspection 2026-05-10: call site `self._scan_task_specs_for_agents_md_drift()` no longer present in `src/daemon/handlers/idle.py`. Method definition retained at idle.py:85 (still consumed by MCP `validate_task_spec` at upload time) but no longer invoked on IDLE-tick. Event log no longer shows `[AGENTS-SCAN]` spam. **Historical context preserved for reference:** PR-260 introduced periodic anti-pattern scan over `tasks/PR-*.md` at IDLE cycle time emitting `[AGENTS-SCAN]` events on violations. Events advisory only — they recommend operator review but do NOT block dispatch, do NOT gate any workflow, have NO associated remediation surface. Production observation 2026-05-07: 7+ violations across spec files at any given cycle, each producing 3-4 line entry in event log. With 100-entry event log history cap, AGENTS-SCAN spam pushed real operational signals out of view. False positives also occurred because `_ANTI_PATTERNS` regexes lacked fenced-code-block + inline-backtick suppression. MICRO PR removed the IDLE-tick invocation. Method body + `_ANTI_PATTERNS` table preserved (still consumed at upload time by MCP). Status: CLOSED.
...
### New OBS items (added 2026-05-10 from Sprint 15c v4 reconciliation session)

- **OBS-CY** (dashboard upload route silently overwrites status:DONE frontmatter on collision): **OPEN, medium severity, observed 2026-05-10**. Dashboard `/upload-tasks` route extracts uploaded zip and replaces matching files in `tasks/` directory without checking existing frontmatter `status:` field. When the zip is regenerated by LLM client (e.g. v4 spec set from chat session), specs that have status:DONE on disk (because corresponding PR already merged on GitHub) get reset to status:TODO if the zip contains them with default frontmatter. Production observation 2026-05-10: v4 zip upload caused PR-289a, PR-289b-1, PR-289b-2, PR-293..298 (8 specs) to revert from status:DONE to status:TODO. Daemon picker then immediately re-dispatched PR-289a on next IDLE cycle, attempting to re-implement an already-merged spec. Coder identified the conflict in PLANNED PR output (`applying the exact one-pattern skeleton would revert subsequent in-scope guardrails`) but could not escalate cleanly. **Fix scope (PR-FUTURE-UPLOAD-PRESERVE):** dashboard upload handler in `src/web/api/sprint_upload.py` (or equivalent) merges existing frontmatter with new body on collision. Logic: if existing `status ∈ {DONE, merged, in_review, in_progress}` → preserve existing frontmatter, log warning event, overwrite body only; if existing `status ∈ {TODO, blocked, canceled, error}` or status absent → overwrite normally. ~50 LoC, ~1 hour. **Sprint 16 Batch A "Reliability layer".**

- **OBS-CZ** (dashboard upload waits for IDLE state with no operator visibility into pending uploads): **OPEN, low severity, observed 2026-05-10**. Dashboard upload route accepts the zip immediately, displays "Uploaded N files" success event, but the actual disk write + git commit happens during next daemon IDLE-tick (because operations on tasks/ directory race with daemon's working tree access during CODING/FIX/WATCH cycles). Production observation 2026-05-10: operator uploaded v4 zip while daemon was in CODING state on a prior task. Upload route extracted zip into working tree but did not commit. Operator restarted daemon container before next IDLE tick, losing the uncommitted extracted files. No operator-visible signal that "files extracted but not yet committed" state existed. **Mitigation:** add "Upload pending IDLE" status badge to dashboard upload result modal, possibly with explicit Cancel option to discard pending uncommitted extract. Alternative: upload route blocks synchronously waiting for IDLE state up to N seconds before responding. ~30 LoC + UI work, Sprint 16 Batch A.

- **OBS-DA** (MCP server has no `get_repo_task_status` tool for client-side conflict prevention): **OPEN, low-medium severity, observed 2026-05-10**. MCP Orchestrator v1 tools cover spec validation (`validate_task_spec`), schema retrieval (`get_task_schema`), AGENTS.md template (`get_agents_md_template`), next PR number suggestion (`suggest_next_pr_number`), and healthcheck. LLM clients preparing task spec zips cannot programmatically detect "this spec id is already merged on GitHub and should not be in the regenerated zip" before upload. Production observation 2026-05-10: this session's v4 zip generation included PR-289a, PR-289b-1, PR-289b-2, PR-293..298 with default `status: TODO` frontmatter because LLM client had no way to query disk state. **Fix scope (PR-FUTURE-MCP-STATUS):** add MCP tool `get_repo_task_status(repo_slug)` that returns map of `{spec_id: {status, merged_pr_number_if_known}}` by scanning `tasks/PR-*.md` frontmatter on disk in the daemon's mounted volume. LLM client calls before zip preparation, filters out specs with `status ∈ {merged, DONE}`, and either skips them or marks them in the zip with preserve-frontmatter directive (combined with OBS-CY fix). ~50 LoC, ~1 hour. **Sprint 16 Batch A "Reliability layer".**

- **OBS-DB** (daemon picker has no merged-PR sanity check before dispatching coder on status:TODO spec): **OPEN, medium severity, regression-class observation 2026-05-10**. Daemon picker reads `tasks/PR-*.md` frontmatter, finds eligible TODO specs (deps satisfied), and dispatches without verifying that the corresponding branch is already merged on GitHub. This is a fresh failure surface for the OBS-CT class (closed 2026-05-07 by MICRO #344's GraphQL probe fix for already-shipped PRs): GraphQL probe works for sane disk state, but if status frontmatter is locally TODO and the branch was merged 24+ hours ago by a previous session, picker does not query GraphQL until WATCH cycle by which point coder has already been dispatched. Production observation 2026-05-10: after v4 zip upload reset PR-289a to status:TODO (per OBS-CY), picker immediately dispatched PR-289a on next IDLE tick. Coder identified the conflict in PLANNED PR output but could not escalate cleanly; operator manually stopped via UI. **Possible mitigations:** (a) picker queries GraphQL merged-set for the corresponding branch before dispatch — adds 1 API call per dispatched task, well within quota; (b) Sprint 15b Phase 2 frontmatter status writer lands and writes status:merged on merge commit, eliminating the disk-state TODO regression entirely; (c) Sprint 16 PR-FUTURE-UPLOAD-PRESERVE + PR-FUTURE-MCP-STATUS prevent the regression at upload time. **Decision deferred to next session** — likely (b) + (c) together suffice without picker-level fix needed.

- **PR-FUTURE-RESTART-BUTTON** (~145 LoC, Sprint 16 Batch A): runtime reset endpoint + UI button. Behavior: abort current daemon operation (similar to user_stop but softer), wait IDLE state with 30s timeout, clear runtime Redis keys (`{slug}:state`, `cancellation:{slug}:*`, current_task pointer), reset workdir to origin/main via `git fetch + reset --hard origin/main` (handles uncommitted upload droppings from OBS-CZ class incidents), re-scan tasks/ from disk for next picker tick. UI: red "Restart" button next to Pause/Stop with confirm dialog. What restart does NOT touch: `tasks/PR-*.md` frontmatter, `done_index` Redis set, event log history, repo config. This is a runtime reset, not content factory reset. Component breakdown: frontend button + modal (~30 LoC), API endpoint (~20 LoC), runner abort+reset method (~30 LoC), Redis cleanup helper (~15 LoC), tests (~50 LoC). Total ~145 LoC.

- **PR-FUTURE-UPLOAD-PRESERVE** (~50 LoC, Sprint 16 Batch A): dashboard upload preserves DONE/merged/in_review frontmatter. See OBS-CY above for full description.

- **PR-FUTURE-MCP-STATUS** (~50 LoC, Sprint 16 Batch A): MCP tool returns spec status map for client-side conflict prevention. See OBS-DA above for full description.

- **Sprint 16 Batch A "Reliability layer"** (combined ~5 hours, 3 PRs): defensive infrastructure to prevent class of v3→v4 reconciliation chaos observed 2026-05-10. RESTART-BUTTON gives operator clean recovery path when state desyncs; UPLOAD-PRESERVE prevents the most common desync at upload time; MCP-STATUS prevents the desync upstream at zip preparation time. Ships before Sprint 16 main config-architecture work because reliability-layer issues block daily ops more than config layering does.

### New OBS items (added 2026-05-11 from PR-422 race-condition diagnosis session)

- **OBS-DC** (pre-merge sync race against GitHub REST API eventual consistency on `/pulls/{pr}` after daemon-initiated push): **CLOSED 2026-05-11 by PR-422 (commit 9dd4e4a)**. Pre-merge sync flow in `src/daemon/handlers/merge.py:172-210` sequence: (1) `git merge origin/{base}` produces merge commit, (2) `git push origin {pr_branch}` ships it, (3) `gh_cache._invalidate_etag_cache("repos/{repo}/pulls")` clears local cache, (4) `self._post_codex_review(number)` → `_post_codex_review_result` → `gh_prs.get_pr_metadata(...)` → REST API GET `/repos/{repo}/pulls/{pr}` happens within milliseconds of the push. GitHub side eventual consistency on read replicas can return pre-push head_sha/head_commit_date for several seconds. `_author_already_requested_review(after_iso=stale_date)` then matches the operator's recent `@codex review` anchor (created within the 5-minute dedup window for the prior head), suppresses the post with `[INFRA] Skipping duplicate @codex review for PR #{n}; PR author already requested review for this head`. Codex never reviews the merge commit. Body 👍 reaction's `created_at` falls before the new `head_commit_time` so `body_approved` rejects it. Classifier falls through to CHANGES_REQUESTED via the "Didn't find any major issues" issue comment after anchor. WATCH cycle waits review_timeout and ESCALATEs without Codex ever evaluating the merged code. **Verified on PR #420 (2026-05-11):** 👍 reaction at 23:53:53Z, merge commit committer date 23:54:03Z (10s gap), operator anchor at 23:49:10Z. Direct probe hours later confirmed `has_recent_codex_review_request(after_iso=23:54:03)` returns False (fresh data), but daemon log at 23:54:07Z showed `Skipping duplicate` — proving `get_pr_metadata` returned a non-fresh value at the exact moment of the dedup check. **Fix (PR-422):** added `bypass_author_dedup: bool = False` parameter to `_post_codex_review` and `_post_codex_review_result` in `src/daemon/handlers/hung.py`; gated the PR-author-dedup `elif` on `not bypass_author_dedup`; pre-merge sync call site in `merge.py:194-208` passes `bypass_author_dedup=True`. Other call sites (handle_coding, handle_fix, EYES retrigger, stale retrigger) keep default `bypass_author_dedup=False` because they legitimately need protection against AGENTS.md-runbook double-posts. **Production verification:** PR #420 entered MERGE state after operator `@codex review` retrigger, pre-merge sync produced merge commit ef88de7, new log line appeared, `[INFRA] Posted @codex review on PR #420.` followed (not "Skipping duplicate"), Codex reviewed ef88de7 with 👍, WATCH cycle saw APPROVED, daemon merged PR #420 → IDLE → picked PR-317. End-to-end fix validated in one cycle.
...
### New OBS items (added 2026-05-14 from MICRO-1/1b ship + PR-305 disaster + activity API discovery)

- **OBS-DE** (Sprint 15b Phase 2 HUNG removal gap — PENDING state retrigger missing): **CLOSED 2026-05-13 by MICRO-1 (PR-432) + MICRO-1b**. Audit on 2026-05-12 of pre-PR-277 `handle_hung` method body (via `git show 4a68f5f:src/daemon/handlers/hung.py`) found 5 distinct behaviors (B1-B5). Sprint 15b Phase 2 (PR-276/PR-277, shipped 2026-05-08..09) deleted the handler **without explicit migration list in specs**. 3 of 5 behaviors orphaned: B2 external PR MERGED/CLOSED detection (medium), B4b watch_retrigger_cap mechanism (high), B4c PENDING state retrigger (critical — production bug observed on PR #428 2026-05-12 where Codex Connector accepted `@codex review` mention but produced no verdict, daemon waited full 20m review_timeout, ESCALATEd, re-picked, looped without retrigger). Plus B5 dead config flag `hung_fallback_codex_review` (low). MICRO-1 (PR-432) extended `_maybe_retrigger_stale_review` in `src/daemon/handlers/watch.py:639` to handle PENDING (gated by existing `hung_fallback_codex_review` flag, default True) and pass `bypass_author_dedup=True` to the post call. Added PENDING dispatch in `handle_watch`. MICRO-1b shipped cap mechanism: async migration of `_maybe_retrigger_stale_review`, cap check via `current_pr.watch_retrigger_count >= app_config.daemon.watch_retrigger_cap` (default 3), `await self._commit_and_park_in_error(subsource="watch_retrigger_cap", ...)` on cap reached, counter increment after successful post. B5 config flag re-wired via PENDING gate. **CRITICAL CAVEAT 2026-05-14:** Both MICRO-1 and MICRO-1b silently non-functional in production due to OBS-DI (activity API CLI flag bug, see below). MICRO-1 fix was applied to a function path that always returns None on production. Fix functional only at code-path level; runtime behavior remains broken until OBS-DI shipped. B2 (external PR resolution detection) remains open as future MICRO scope ~30 LoC. **Cross-reference:** gap report `/mnt/user-data/outputs/gap-report-hung-removal-2026-05-12.md` (session document) enumerates all 5 behaviors with verdicts.

- **OBS-DF** (large-diff guardrail produces 8x false positives on legitimate work): **OPEN, disabled 2026-05-14 via config.yml override on AI-Server**. Statistical analysis of last 200 merged PRs on 2026-05-14: P50=330 LoC, P75=703, P90=1255, **P95=1760** (current threshold 1500 sits at approximately P92), P99=7983, max=20300. 8% of legitimately merged PRs trip the threshold — file splits (PR-224a/b: 20300+7983 LoC moves), substantial features (PR-283 Retry button: 2863 LoC, PR-300 commit audit: 1734 LoC), migration work (PR-315 cancellation collapse: 1642 LoC). Static threshold cannot distinguish "legitimate big work" from "bloated small spec" (the real bloat signal). Today's PR-305 disaster confirmed: spec estimated ~250 LoC, coder produced 1535 LoC, guardrail tripped — true positive. But same threshold trips on legitimate refactors. **Workaround applied 2026-05-14:** added to `config.yml` daemon section `large_diff_addition_threshold: 999999999` and `large_diff_files_threshold: 999999999`. Other 3 Tier 2 guardrails (secrets PR-302a/b/c, mass-deletion PR-303, governance PR-304) remain active. **Restoration plan:** re-enable with operator-override design when PR-306 dashboard ships (operator clicks approve/reject on flagged PRs via PR-305a/b/c backend, all 3 merged today). Treat guardrail as soft signal not hard wall. Spec-aware ratio (compare PR LoC vs spec's declared estimate) is the long-term correct measure but requires structured spec metadata extension. Backlog item.

- **OBS-DG** (PR-305 monolith disaster — 1535 LoC for ~250 LoC spec): **CLOSED 2026-05-14 via split into PR-305a/b/c, all merged**. PR-305 spec body declared "If implementation exceeds 150 LoC during the first FIX iteration, escalate via stdout — operator will split the spec into smaller follow-up PRs." Coder produced **1535 LoC across 4 files** (10x self-imposed threshold, 6x estimate). Spec self-escalation worked **as designed** — Tier 2 guardrail caught the violation, daemon parked task in ERROR. Operator-driven split is the canonical recovery path. Spec text was correct; coder ignored it. **Lessons:** (a) LLM coders do not respect "approximately N LoC" estimates in spec body — they implement until task feels "complete" regardless; (b) test-heavy specs underestimate by 3-5x because integration test setup is ~50-100 LoC per test, not the assumed ~20 LoC; (c) split-by-functional-boundary worked: PR-305a (storage helper ~170 LoC actual) + PR-305b (GET endpoint ~190 LoC actual) shipped clean within budget; PR-305c (POST endpoint with approve/reject + atomic guard) **also produced 1526 LoC**, still 4x estimate, eventually merged after `large_diff` guardrail disabled (see OBS-DF). **Methodology lesson for spec writing:** integration tests add ±2-4x error bar to LoC estimates that single-purpose unit tests do not. Split-by-functional-boundary is necessary but not sufficient for predictable LoC; for endpoints with substantial test setup (mock GH PR + redis state + filesystem fixture + git repo), split further by sub-feature (approve branch / reject branch / atomic guard as 3 sub-PRs).

- **OBS-DH** (diagnose_error infinite loop after max_attempts reached): **OPEN, high severity, observed 2026-05-14**. After Tier 2 GUARDRAIL trip on PR-305 (PR #439) caused transition to ERROR state, `handle_error` invoked `diagnose_error` which hit `max_attempts (3) reached, staying ERROR` then **continued re-invoking handler each poll cycle**. Log:
  ```
  2026-05-14 05:58:25  [ERROR] diagnose_error: max attempts (3) reached, staying ERROR
  2026-05-14 05:59:24  [ERROR] diagnose_error: max attempts (3) reached, staying ERROR
  [... 331 iterations over 7+ hours ...]
  ```
  Expected behavior: after max_attempts reached → park task → **stop calling handler**. Actual: continues calling handler every poll cycle ~1/min. Wastes minimal API budget (no external calls in diagnose path) but never self-recovers — only Retry button or daemon restart breaks the loop. **Trigger:** any task transitioning to ERROR state with `error_handler_use_ai=true` enabled. **Fix scope:** add "diagnose_attempts >= max_attempts" gate in handle_error dispatch; after max_attempts logged once, set sentinel in Redis (e.g. `diagnose_exhausted:{repo}:{task_id}`) that prevents re-invocation until operator action (Retry button clears it). ~15 LoC + 2 tests. **Files involved:** `src/daemon/handlers/error.py` (diagnose_error logic), `src/daemon/runner.py` (poll dispatch). **Sprint 15c Batch A urgent fix.**

- **OBS-DI** (`get_pr_last_push_time` uses wrong gh CLI flag syntax — function broken since ship): **OPEN, CRITICAL severity, observed 2026-05-14**. `src/github/prs.py:get_pr_last_push_time` uses `-f key=value` flags for GitHub repository activity API:
  ```python
  date_raw = gh_runner.run_gh([
      "api", f"repos/{repo}/activity",
      "-f", f"ref=refs/heads/{branch}",
      "-f", "activity_type=push",
      "-f", "per_page=1", "-f", "direction=desc",
  ])
  ```
  `gh api -f` sends form-encoded body (correct for POST). Activity API is **GET** endpoint, requires URL query syntax. With `-f` returns 404 always. **Verified via direct tests on 2026-05-14:**
  ```
  gh api repos/.../activity                                    → 30 items ✓
  gh api repos/.../activity -f ref=refs/heads/main             → 404 ✗
  gh api 'repos/.../activity?ref=refs/heads/main&...&per_page=3' → 3 items ✓
  ```
  **Cascading impact:** `_maybe_retrigger_stale_review` calls `get_last_push_age_seconds` → calls `get_pr_last_push_time` → returns None always → silent early return at "if last_push_age_seconds is None: return False". All MICRO-1 PENDING retrigger functionality **broken globally** since MICRO-1 ship 2026-05-13. All CHANGES_REQUESTED stale retrigger broken globally. All EYES stale retrigger broken globally. Pre-PR-277 HUNG retry worked because simpler (no activity API dependency, just `hung_fallback_codex_review` config flag gate). **Claude (spec author) introduced this regression in MICRO-1 spec** — added `last_push_age_seconds` check as "extra defense" without verifying function works on production. Tests passed because mocked `gh_prs.get_last_push_age_seconds` to return fake numbers; mock never tested CLI subprocess path. **Fix scope:** replace `-f` flags with URL query string. ~5 LoC change in `get_pr_last_push_time` + integration test at gh_runner level that asserts exact CLI args passed. **CRITICAL priority** — entire retrigger infrastructure (OBS-DE behaviors B4a/B4b/B4c) silently non-functional in production. **Files involved:** `src/github/prs.py:get_pr_last_push_time`. **Sprint 15c Batch A urgent fix.**

- **OBS-DJ** (silent early returns in `_maybe_retrigger_stale_review` — no observability on skip reasons): **OPEN, medium severity, observed 2026-05-14**. `src/daemon/handlers/watch.py:_maybe_retrigger_stale_review` has 4 early `return False` paths without log events:
  1. `if current_pr is None: return False` (no log)
  2. `if review_status not in (CHANGES_REQUESTED, EYES, PENDING): return False` (no log)
  3. `if last_push_age_seconds is None: return False` (no log — hit production silently due to OBS-DI for ~24h before discovery)
  4. `if last_push_age_seconds < stale_after.total_seconds(): return False` (no log)
  Function exits silently with no events in daemon event log. Production debugging of OBS-DI took ~3 hours on 2026-05-14 because **no signals were emitted** that retrigger gate hit and rejected. **Lesson:** all early returns in critical-path functions should log at least once per cycle (or per pr_number) the reason for skip. **Fix scope:** add `self.log_event(...)` before each `return False` with concise reason string. ~10 LoC + 4 tests asserting log lines appear. Bundle with OBS-DI fix in same MICRO PR — same file, related concern. **Sprint 15c Batch A.**

- **B2 gap reconfirmed** (external PR MERGED/CLOSED detection missing post-Sprint 15b Phase 2): **OPEN, medium severity, re-observed 2026-05-14**. Operator manually closed PR #443 via `gh pr close` during PR-305c debugging. Daemon **did not detect** PR state change. Continued running WATCH cycle on closed PR, hitting timeout → ESCALATE → re-pick → WATCH again. Loop did not stop until Redis state reset. Pre-PR-277 `handle_hung` method polled `gh pr view --json state` every cycle and transitioned HUNG → IDLE on MERGED/CLOSED detection. PR-277 removed handler without migration of this behavior. Same gap as listed in OBS-DE (B2 sub-behavior), confirmed in production today. **Fix scope:** add periodic `gh pr view --json state` poll in WATCH (or IDLE) handler. Transition to IDLE on MERGED/CLOSED state with `current_task=None` and clear `current_pr`. ~30 LoC + 2 tests. **Sprint 15c Batch A future MICRO PR.**

### New OBS items (added 2026-05-15 from PR-321 ship and OBS-DK regression discovery)

- **OBS-DK** (`monitor_fix_idle` baseline corruption from PR-321 contract change): **OPEN, CRITICAL severity, observed 2026-05-15**. `src/daemon/fix_supervision.py:monitor_fix_idle` (lines 51-61) initializes the FIX idle timer baseline using `gh_prs.get_last_push_age_seconds`:
  ```python
  ...
  if head_age is not None:
      ...
      last_known_push = now
  ```
  **Before PR-321:** `get_last_push_age_seconds` was broken (OBS-DI, returned None always). Code took the `else: last_known_push = now` branch. FIX got full `idle_limit=1800s` window. Bug masked.
  **After PR-321 (commit 32d1061, shipped 2026-05-14→15):** function returns real values (6-8 hours = 25000+ seconds for PRs entering FIX after normal review/CI cycle). `backdate = min(28000, 1740) = 1740`. `last_known_push = now - 1740`. First poll cycle after 60s → `elapsed = 60 - (-1740) = 1800` → `elapsed >= idle_limit` → kill within 60 seconds of FIX start.
  **Production verified 2026-05-15:**
  ```
  PR #79 megaraid-dashboard:    head_age: 28802 seconds = 480.0 minutes
  PR #451 pipeline-orchestrator: head_age: 23276 seconds
  ```
  Both >> `idle_limit - poll_interval = 1740s` → immediate kill on every FIX iteration. STDOUT/STDERR empty because Claude never got chance to output. Log message says "1800s since last push" but only 60s elapsed since FIX entry.
  **Root cause analysis:** the `backdate` mechanism was designed for daemon restart mid-FIX scenarios — if daemon was down 5 minutes during a FIX iteration, `head_age` would be ~5 minutes and backdating preserved elapsed FIX time correctly. For the common case of FIX starting after a normal review/CI cycle (which takes hours), `head_age` is many hours and backdating is wrong — FIX is starting fresh, not resuming an in-flight iteration.
  **Fix scope:** change baseline init to backdate only when `head_age < poll_interval` (i.e., daemon restart case). Otherwise `last_known_push = now`. ~5 LoC change in `src/daemon/fix_supervision.py:51-61` plus 4 tests (small head_age backdates, large head_age starts fresh, None head_age starts fresh, regression test reproducing 2026-05-15 incident).
  **Files involved:** `src/daemon/fix_supervision.py:monitor_fix_idle`.
  **Sprint 15c Batch A urgent fix** — entire FIX iteration infrastructure broken until ship.
  **Cross-reference:** Codex prompt for fix drafted in 2026-05-15 session.

- **OBS-DL** (PR-072 megaraid task complexity exceeds single Claude AUTO PR cycle): **OPEN, medium severity, design issue not bug, observed 2026-05-15**. Megaraid-dashboard PR-072 spec defines a major refactor — "Coverage to 100% (web + app + entry points)" with 9 new test files covering 196 statements with branch coverage, 10 acceptance criteria including ruff+mypy+pytest+ci.sh, no `# pragma: no cover` allowed. Spec type=refactor complexity=high. Dependencies (PR-067/068/069/070) all DONE so picker correctly dispatched. Claude in CODING state ran full `planned_pr_timeout_sec=3600` (60 minutes) without producing any STDOUT/STDERR output. Manual reproduction confirmed: `claude --print --dangerously-skip-permissions "AUTO PR\nTask: PR-072\n..."` with same env/cwd as daemon hangs 60+ seconds without output; trivial "AUTO PR ... Just print hello and exit." returns "Hello" immediately. Task body content ~3000 chars, AGENTS.md 293 lines — not size issue. Task complexity exceeds single Claude AUTO PR cycle capacity.
  **Not a daemon bug.** Resolution path 2026-05-15: operator implements PR-072 in megaraid chat session manually.
  **Long-term fix:** new spec authoring rule — `Complexity: high` triggers mandatory split planning before upload, do not rely on Claude self-escalation. Split high-complexity refactor tasks into per-module sub-PRs (e.g., PR-072 → PR-072a routes_storcli_errors ~300 LoC, PR-072b csrf_branches ~150 LoC, PR-072c rate_limit_branches ~200 LoC, PR-072d routes_validation_errors ~150 LoC, PR-072e app_lifespan ~150 LoC). Each sub-PR fits single Claude cycle.
  **Generalization of OBS-DG methodology lesson:** complexity:high tasks require operator-driven split during spec writing, not at first failure.

- **OBS-DM** (split parent dependency orphan pattern): **OPEN, medium severity, observed 2026-05-15**. When a parent task `PR-N` is split into `PR-Na/Nb/Nc` and the parent task file is deleted from `tasks/`, other tasks with `Depends on: PR-N` become silently blocked forever — picker cannot resolve `PR-N` because (a) task file no longer exists, (b) merged PR pr_id may be `PR-Nc` not `PR-N`.
  **Observed 2026-05-15:** PR-306 had `Depends on: PR-305, PR-320`. PR-305 was split into PR-305a/b/c on 2026-05-14 and parent file deleted. All three sub-tasks merged. PR-306 marked blocked in dashboard ("Blocked by: PR-305") despite full dependency chain complete. Same pattern affected PR-308a (`Depends on: PR-307` after PR-307 → PR-307a/b split) and PR-310 (`Depends on: PR-306` transitively blocked).
  **Manual fix applied 2026-05-15:**
  ```
  sed -i 's/Depends on: PR-305, PR-320/Depends on: PR-305c, PR-320/' tasks/PR-306.md
  sed -i 's/Depends on: PR-307, PR-320/Depends on: PR-307b, PR-320/' tasks/PR-308a.md
  ```
  Pattern: update reference to last sub-task in chain.
  **Long-term fix:** picker should resolve `Depends on: PR-N` through any of `PR-N`, `PR-Na`, `PR-Nb`, ..., `PR-Nz` — last sub-task DONE → parent dependency satisfied.
  **Fix scope:** ~10 LoC change in `_filter_dag_headers_with_available_dependencies` (idle.py:157-191) plus 2 tests (PR-N matches PR-Na/b/c when parent absent; PR-N still matches PR-N directly when parent present).
  **Sprint 16 Batch A or earlier MICRO PR.**

- **OBS-DN — meta-pattern: spec dependency contract verification missing** (HIGH, methodology fix): **OPEN, observed 2026-05-15 as accumulated pattern from MICRO-1 (2026-05-13) and PR-321 (2026-05-15)**. Both regressions had identical root cause: spec author added new behavior to a function or changed return value semantics without auditing callers' implicit contract assumptions. MICRO-1 spec referenced `gh_prs.get_last_push_age_seconds` based on mocked test return values, did not verify production behavior — function returned None broken in production, callers silently early-returned. PR-321 fixed the function to return real values, but did not enumerate callers and verify each handled the new value range — `monitor_fix_idle` had implicit assumption that None-or-small was the design space.
  **Direct production costs:**
  - MICRO-1: ~3 hours debugging 2026-05-14 + retrigger infrastructure broken ~24 hours globally before discovery.
  - PR-321: ~7 hours of FIX iterations broken globally between ship and discovery.
  **Mandatory pre-flight for spec changing function behavior or return value range:**
  1. `grep -rn 'function_name'` to enumerate all callers (including indirect via wrapper functions).
  2. For each caller, document in spec's `## Threat model` section: what is the caller's implicit contract assumption (does it tolerate None, does it tolerate large values, does it cache return value)?
  3. Verify each caller behaves correctly under new return value range. If a caller breaks, either fix that caller in the same spec OR add explicit migration note for follow-up.
  4. Add `## Callers audited` subsection to Threat model listing the grep'd callers + the contract verification result for each.
  **Sprint 17 spec authoring AGENTS.md addition** — codify this as mandatory section in `spec_anatomy` template.
